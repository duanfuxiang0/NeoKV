...
struct LogHead {
	explicit LogHead(const rocksdb::Slice& raw) {
		butil::RawUnpacker(raw.data()).unpack64((uint64_t&)term).unpack32((uint32_t&)type);
	}
...
private:
	MyRaftLogStorage(int64_t region_id, RocksWrapper* db, rocksdb::ColumnFamilyHandle* raftlog_handle,
	                 rocksdb::ColumnFamilyHandle* binlog_handle);

	int get_binlog_entry(rocksdb::Slice& raftlog_value_slice, std::string& binlog_value);

	int _build_key_value(SlicePartsVec& kv_raftlog_vec, SlicePartsVec& kv_binlog_vec, const braft::LogEntry* entry,
	                     butil::Arena& arena);
	...
	int _decode_log_data_key(const rocksdb::Slice& data_key, int64_t& region_id, int64_t& index);
