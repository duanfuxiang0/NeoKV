...
public:
	...
	int64_t get_term(int64_t index) const {
		...
		if (_q.size() < 15ul /*FIXME: it's not determined by benchmark*/) {
			...
			for (std::deque<braft::LogId>::const_reverse_iterator iter = _q.rbegin(); iter != _q.rend(); ++iter) {
				if (index >= iter->index) {
					return iter->term;
				}
			}
		} else {
			std::deque<braft::LogId>::const_iterator iter = std::upper_bound(_q.begin(), _q.end(), index, Cmp());
			if (iter == _q.begin()) {
				return 0;
			}
			--iter;
	...
	void truncate_prefix(int64_t first_index_kept) {
		...
		size_t num_pop = 0;
		for (std::deque<braft::LogId>::const_iterator iter = _q.begin() + 1; iter != _q.end(); ++iter) {
