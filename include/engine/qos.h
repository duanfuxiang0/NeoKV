...
enum QosType { QOS_SELECT = 0, QOS_DML = 1, QOS_REVERSE_MERGE = 2, QOS_TYPE_NUM };
...
public:
	...
	void reset_rate(const int64_t rate) {
		int64_t min_global_extended_tokens =
		    FLAGS_max_tokens_per_second * FLAGS_min_global_extended_percent / 100; // 最小全局超额令牌个数
		_valid = true;
		_rate = rate;
		if (rate <= 0) {
			_valid = false;
...
public:
	explicit SqlQos(uint64_t sign)
	    : _sign(sign), _committed_bucket(false), _get_statistics(FLAGS_qps_statistics_minutes_ago * 60LL),
	      _scan_statistics(FLAGS_qps_statistics_minutes_ago * 60LL), _get_real(FLAGS_qos_reject_interval_s),
	      _scan_real(FLAGS_qos_reject_interval_s), _token_fetch(60),
	      _sql_statistics(FLAGS_qps_statistics_minutes_ago * 60LL), _sql_real(FLAGS_qos_reject_interval_s),
	...
	void analyze_qps(int64_t& get_qps, int64_t& scan_qps) {
		...
		if (get_qps != 0 || scan_qps != 0) {
			DB_WARNING("sign: %lu, get_qps: %ld, scan_qps: %ld, sql_qps: %ld,"
			           ...
			           _sign, get_qps, scan_qps, _sql_qps, _get_real.get_qps_value(), _scan_real.get_qps_value(),
			           _token_fetch.get_qps_value(), _concurrency.count(), _concurrency.total_count());
		}
	...
	void set_need_reject(const uint64_t index_id) {
		_reject_index_id = index_id;
	}
...
public:
	...
	void analyze_qps(int64_t& total_get_qps, int64_t& total_scan_qps) {
		...
		DB_WARNING("total_get_qps: %ld, total_scan_qps: %ld, global select concurrency: %d", total_get_qps,
		           total_scan_qps, Concurrency::get_instance()->global_select_concurrency.count());
	}
...
private:
	StoreQos() : _globle_extended_bucket(true, FLAGS_max_tokens_per_second), _bthread_local_key(INVALID_BTHREAD_KEY) {
	}
