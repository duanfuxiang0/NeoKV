...
enum KVMode { KEY_ONLY, VAL_ONLY, KEY_VAL };

enum GetMode { GET_ONLY, LOCK_ONLY, GET_LOCK };

class KeyPointsTblPropCollector : public rocksdb::TablePropertiesCollector {
public:
	const char* Name() const override {
		return "KeyPointsTblPropCollector";
	}
	...
	rocksdb::Status Finish(rocksdb::UserCollectedProperties* properties) override {
		if (_value_encoded.size() > 0) {
			*properties =
			    rocksdb::UserCollectedProperties {{"key_point_lens", _size_encoded}, {"key_points", _value_encoded}};
			DB_WARNING(
...
class KeyPointsTblPropCollectorFactory : public rocksdb::TablePropertiesCollectorFactory {
public:
	explicit KeyPointsTblPropCollectorFactory() {
	}

	rocksdb::TablePropertiesCollector*
	CreateTablePropertiesCollector(TablePropertiesCollectorFactory::Context context) override {
...
public:
	...
	void set_table_options(rocksdb::BlockBasedTableOptions& table_options, const pb::RocksdbGFLAGS& rocksdb_gflags);
	rocksdb::Status write(const rocksdb::WriteOptions& options, rocksdb::WriteBatch* updates) {
		return _txn_db->Write(options, updates);
	}
	rocksdb::Status write(const rocksdb::WriteOptions& options, rocksdb::ColumnFamilyHandle* column_family,
	...
	rocksdb::Status put(const rocksdb::WriteOptions& options, rocksdb::ColumnFamilyHandle* column_family,
	                    const rocksdb::Slice& key, const rocksdb::Slice& value) {
		return _txn_db->Put(options, column_family, key, value);
	}

	rocksdb::Status merge(const rocksdb::WriteOptions& options, rocksdb::ColumnFamilyHandle* column_family,
	                      const rocksdb::Slice& key, const rocksdb::Slice& value) {
		return _txn_db->Merge(options, column_family, key, value);
	}

	rocksdb::Transaction* begin_transaction(const rocksdb::WriteOptions& write_options,
	                                        const rocksdb::TransactionOptions& txn_options) {
		return _txn_db->BeginTransaction(write_options, txn_options);
	}

	rocksdb::Transaction* begin_cold_transaction(const rocksdb::WriteOptions& write_options,
	                                             const rocksdb::TransactionOptions& txn_options) {
	...
	rocksdb::Status compact_files(const rocksdb::CompactionOptions& options, rocksdb::ColumnFamilyHandle* column_family,
	                              std::vector<std::string> input_file_names, int32_t output_level) {
		return _txn_db->CompactFiles(options, column_family, input_file_names, output_level);
	}

	rocksdb::Status flush(const rocksdb::FlushOptions& options, rocksdb::ColumnFamilyHandle* column_family) {
		return _txn_db->Flush(options, column_family);
	}

	rocksdb::Status remove(const rocksdb::WriteOptions& options, rocksdb::ColumnFamilyHandle* column_family,
	...
	rocksdb::Status remove_range(const rocksdb::WriteOptions& options, rocksdb::ColumnFamilyHandle* column_family,
	                             const rocksdb::Slice& begin, const rocksdb::Slice& end, bool delete_files_in_range);
	...
	rocksdb::Iterator* new_iterator(const rocksdb::ReadOptions& options, rocksdb::ColumnFamilyHandle* family) {
	...
	rocksdb::Status ingest_to_cold(const std::vector<std::string>& external_files) {
		...
		options.allow_blocking_flush = false; // 设置为false时，如果和memtable有重叠则会失败，cold
		                                      // rocksdb不会写入，不会产生memtable所以设置为false也安全
		...
		options.fail_if_not_bottommost_level =
		    true; // 我们ingest的sst没有区间重合，理论上会ingest到最后一层，所以设置为true进行校验
#endif
		if (_cold_txn_db != nullptr) {
	...
	rocksdb::Status ingest_offline_binlog_sst(const std::vector<std::string>& external_files,
		...
		// 我们ingest的sst没有区间重合，理论上会ingest到最后一层，所以设置为true进行校验
		if (_cold_txn_db != nullptr) {
			if (is_binlog_cf) {
