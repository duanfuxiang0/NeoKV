...
public:
	...
	void MultiGet(const rocksdb::ReadOptions& options, rocksdb::ColumnFamilyHandle* column_family,
	              const std::vector<rocksdb::Slice>& keys, std::vector<rocksdb::PinnableSlice>& values,
	              std::vector<rocksdb::Status>& statuses, bool sorted_input);

	rocksdb::Status GetForUpdate(const rocksdb::ReadOptions& options, rocksdb::ColumnFamilyHandle* column_family,
	                             const rocksdb::Slice& key, std::string* value);
	...
	rocksdb::Status Merge(rocksdb::ColumnFamilyHandle* column_family, const rocksdb::Slice& key,
	                      const rocksdb::Slice& value, const bool assume_tracked = false);
	...
	rocksdb::Status Delete(rocksdb::ColumnFamilyHandle* column_family, const rocksdb::SliceParts& key) {
		COLD_DB_CHECK_WRITE;
		return _txn->Delete(column_family, key);
	}

	rocksdb::Status SetName(const rocksdb::TransactionName& name) {
		return _txn->SetName(name);
	}

	rocksdb::TransactionName GetName() const {
		return _txn->GetName();
	}

	rocksdb::TransactionID GetID() const {
		return _txn->GetID();
	}
	...
	rocksdb::Status RollbackToSavePoint() {
		return _txn->RollbackToSavePoint();
	}

	std::vector<rocksdb::TransactionID> GetWaitingTxns(uint32_t* column_family_id, std::string* key) const {
		return _txn->GetWaitingTxns(column_family_id, key);
	}

	void DisableIndexing() {
		_txn->DisableIndexing();
	}

	bool use_cold_db() const {
		return _use_cold_db;
	}
