...
public:
	...
	void add_all_learner_for_store(const std::string& instance, const IdcInfo& instance_idc,
	                               const std::vector<int64_t>& learner_ids);
	void pre_process_remove_peer_for_store(const std::string& instance, const IdcInfo& instance_idc, pb::Status status,
	                                       std::vector<pb::RaftControlRequest>& requests);
	void pre_process_add_peer_for_store(const std::string& instance, const IdcInfo& instance_idc, pb::Status status,
	                                    std::unordered_map<std::string, std::vector<pb::AddPeer>>& add_peer_requests);
	bool add_region_is_exist(int64_t table_id, const std::string& start_key, const std::string& end_key,
	...
	void leader_heartbeat_for_region(const pb::StoreHeartBeatRequest* request, pb::StoreHeartBeatResponse* response);
	void check_whether_update_region(int64_t region_id, bool peer_changed, const std::string& leader,
	                                 const pb::LeaderHeartBeat& leader_region,
	                                 const SmartRegionInfo& master_region_info, pb::StoreHeartBeatResponse* response);
	...
	void check_peer_count(int64_t region_id, const pb::LeaderHeartBeat& leader_region,
	                      ...
	                      std::set<int64_t>& binlog_table_ids, int32_t table_pk_prefix_dimension,
	...
	void check_whether_illegal_peer(const pb::StoreHeartBeatRequest* request, pb::StoreHeartBeatResponse* response);

	void leader_load_balance(bool whether_can_decide, bool load_balance, const pb::StoreHeartBeatRequest* request,
	                         pb::StoreHeartBeatResponse* response);

	void leader_main_logical_room_check(const pb::StoreHeartBeatRequest* request, pb::StoreHeartBeatResponse* response,
	                                    IdcInfo& leader_idc, std::unordered_map<int64_t, int64_t>& table_replica,
	                                    std::unordered_map<int64_t, IdcInfo>& table_main_idc,
	                                    std::set<int64_t>& trans_leader_region_ids);

	void leader_load_balance_on_pk_prefix(
	...
	void pk_prefix_load_balance(const std::unordered_map<std::string, int64_t>& add_peer_counts,
	                            ...
	                            const std::string& instance, std::unordered_map<int64_t, IdcInfo>& table_balance_idc,
...
public:
	...
	void get_region_ids(const std::string& instance, std::unordered_map<int64_t, std::set<int64_t>>& table_region_ids) {
		BAIDU_SCOPED_LOCK(_instance_region_mutex);
		if (_instance_region_map.find(instance) == _instance_region_map.end()) {
			return;
		}
	...
	void print_region_ids(const std::string& instance) {
		...
		for (auto& table_id : _instance_region_map[instance]) {
			for (auto& region_id : table_id.second) {
				DB_WARNING("table_id: %ld, region_id: %ld in store: %s", table_id.first, region_id, instance.c_str());
			}
		}
	}
	...
	void remove_error_peer(const int64_t region_id, const std::set<std::string>& resource_tags,
	                       std::set<std::string> peers, std::vector<pb::PeerStateInfo>& recover_region_way);
	void remove_illegal_peer(const int64_t region_id, const std::set<std::string>& resource_tags,
	                         std::set<std::string> peers, std::vector<pb::PeerStateInfo>& recover_region_way);
	void recovery_single_region_by_set_peer(const int64_t region_id, const std::set<std::string>& resource_tags,
	...
	void recovery_single_region_by_init_region(const std::set<int64_t> region_ids, std::vector<Instance>& instances,
	                                           std::vector<pb::PeerStateInfo>& recover_region_way);
	...
	void set_region_info(const pb::RegionInfo& region_info) {
		...
		}
		{
			...
			for (auto& learner : region_info.learners()) {
				DB_DEBUG("set region learner instance %s table_id %ld region_id %ld", learner.c_str(), table_id,
				         region_id);
	...
	void set_region_mem_info(int64_t region_id, int64_t new_log_index, int64_t used_size, int64_t num_table_lines) {
		SmartRegionInfo region_ptr = _region_info_map.get(region_id);
		if (region_ptr != nullptr) {
	...
	void erase_region_info(const std::vector<int64_t>& drop_region_ids, std::vector<int64_t>& result_region_ids,
	                       std::vector<int64_t>& result_partition_ids, std::vector<int64_t>& result_table_ids,
	                       std::vector<std::string>& result_start_keys, std::vector<std::string>& result_end_keys);
	...
	std::string construct_max_region_id_key() {
		std::string max_region_id_key =
		    MetaServer::SCHEMA_IDENTIFY + MetaServer::MAX_ID_SCHEMA_IDENTIFY + SchemaManager::MAX_REGION_ID_KEY;
