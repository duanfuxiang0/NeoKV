...
struct Instance {
	...
	Instance(const pb::InstanceInfo& instance_info)
	    : address(instance_info.address()), capacity(instance_info.capacity()),
	      // 若请求中没有该字段，为了安全起见
	      used_size(instance_info.capacity()), resource_tag(instance_info.resource_tag()),
	      physical_room(instance_info.physical_room()), logical_room(instance_info.logical_room()),
...
public:
	...
	void process_cluster_info(google::protobuf::RpcController* controller, const pb::MetaManagerRequest* request,
	...
	// 排除exclude
	int select_instance_rolling(const IdcInfo& idc, const std::set<std::string>& exclude_stores,
	                            std::string& selected_instance);
	int select_instance_min(const IdcInfo& idc, const std::set<std::string>& exclude_stores, const int64_t& table_id,
	                        std::string& selected_instance, const int64_t& average_count = 0);
	int select_instance_min_on_pk_prefix(const IdcInfo& idc_str, const std::set<std::string>& exclude_stores,
	                                     const int64_t& table_id, const std::string& pk_prefix_key,
	                                     std::string& selected_instance, const int64_t& pk_prefix_average_count,
	                                     const int64_t& table_average_count, bool need_both_below_average = false);
	...
	bool logical_and_physical_room_valid(const std::string& logical_room, const std::string& physical_room) {
		...
		if (!physical_room.empty() && logical_iter->second.find(physical_room) == logical_iter->second.end()) {
			return false;
		}
		return true;
	}
...
public:
	...
	void get_instance_count_for_all_level(const IdcInfo& idc,
		...
		for (auto& address : _resource_tag_instance_map[idc.resource_tag]) {
			if (_instance_info.count(address) == 0 || _instance_info[address].resource_tag != idc.resource_tag) {
				continue;
			}
	...
	int64_t get_instance_count(const std::string& resource_tag, std::map<std::string, int64_t>* room_count = nullptr) {
		int64_t count = 0;
		BAIDU_SCOPED_LOCK(_instance_mutex);
		if (_resource_tag_instance_map.count(resource_tag) == 0) {
			return count;
	...
	bool check_resource_tag_exist(const std::string& resource_tag) {
		BAIDU_SCOPED_LOCK(_instance_mutex);
		return _resource_tag_instance_map.count(resource_tag) == 1 && !_resource_tag_instance_map[resource_tag].empty();
	}
	...
	void sub_peer_count(const std::string& instance, int64_t table_id) {
		auto call_func = [instance,
		                  table_id](std::unordered_map<std::string, InstanceSchedulingInfo>& scheduling_info) -> int {
	...
	void sub_peer_count_on_pk_prefix(const std::string& instance, int64_t table_id, const std::string& pk_prefix) {
		auto call_func = [instance, table_id,
		                  pk_prefix](std::unordered_map<std::string, InstanceSchedulingInfo>& scheduling_info) -> int {
			scheduling_info[instance].regions_count_map[table_id]--;
...
private:
	...
	std::string construct_logical_key() {
		return MetaServer::CLUSTER_IDENTIFY + MetaServer::LOGICAL_CLUSTER_IDENTIFY + MetaServer::LOGICAL_KEY;
	}
	std::string construct_physical_key(const std::string& logical_key) {
		return MetaServer::CLUSTER_IDENTIFY + MetaServer::PHYSICAL_CLUSTER_IDENTIFY + logical_key;
	}
	std::string construct_instance_key(const std::string& instance) {
		return MetaServer::CLUSTER_IDENTIFY + MetaServer::INSTANCE_CLUSTER_IDENTIFY + instance;
	}
	std::string construct_instance_param_key(const std::string& resource_tag_or_address) {
		return MetaServer::CLUSTER_IDENTIFY + MetaServer::INSTANCE_PARAM_CLUSTER_IDENTIFY + resource_tag_or_address;
	}
	int load_instance_snapshot(const std::string& instance_prefix, const std::string& key, const std::string& value);
	int load_instance_param_snapshot(const std::string& instance_param_prefix, const std::string& key,
	                                 const std::string& value);
	int load_physical_snapshot(const std::string& physical_prefix, const std::string& key, const std::string& value);
	int load_logical_snapshot(const std::string& logical_prefix, const std::string& key, const std::string& value);
