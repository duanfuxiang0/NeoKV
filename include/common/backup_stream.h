...
protected:
	void multi_iobuf_action(brpc::StreamId id, butil::IOBuf* const messages[], size_t all_size, size_t* index_ptr,
	                        const std::function<size_t(butil::IOBuf* const message, size_t size)>& read_action,
...
public:
	bool set_info(const BackupInfo& backup_info) {
		_meta_file_streaming.open(backup_info.meta_info.path, std::ios::out | std::ios::binary | std::ios::trunc);
		_data_file_streaming.open(backup_info.data_info.path, std::ios::out | std::ios::binary | std::ios::trunc);
