...
struct StoreReqOptions {
	...
	StoreReqOptions()
	    : request_timeout(FLAGS_store_request_timeout), connect_timeout(FLAGS_store_connect_timeout), retry_times(3) {
	}
	StoreReqOptions(const StoreReqOptions& other)
	    : request_timeout(other.request_timeout), connect_timeout(other.connect_timeout),
...
class StoreInteract {
public:
	StoreInteract(const std::string& store_address) : _store_address(store_address), _req_options(StoreReqOptions()) {
	}
	StoreInteract(const std::string& store_address, const StoreReqOptions& req_options)
	    : _store_address(store_address), _req_options(req_options) {
	}
	template <typename Request, typename Response>
	int send_request(uint64_t log_id, const std::string& service_name, const Request& request, Response& response,
		...
		if (cntl.Failed()) {
			DB_WARNING("connect with store fail. send request fail, error:%s, log_id:%lu", cntl.ErrorText().c_str(),
			           cntl.log_id());
			response.set_errcode(pb::EXEC_FAIL);
			return -1;
		}
		if (response.errcode() != pb::SUCCESS) {
			DB_WARNING("send store address fail, log_id:%lu, instance: %s, response:%s, request: %s", cntl.log_id(),
			           _store_address.c_str(), response.ShortDebugString().c_str(), request.ShortDebugString().c_str());
			return -1;
		}
		return 0;
	}
	...
	template <typename Request, typename Response>
	int send_request_for_leader(uint64_t log_id, const std::string& service_name, const Request& request,
	                            Response& response, butil::IOBuf* attachment_data = nullptr) {
		int retry_time = 0;
		do {
			...
			DB_WARNING("connect with store:%s fail. not leader, redirect to :%s,"
			           "log_id: %lu",
			           _store_address.c_str(), response.leader().c_str(), log_id);
	...
	template <typename Request, typename Response>
	int send_request_for_leader(const std::string& service_name, const Request& request, Response& response) {
		uint64_t log_id = butil::fast_rand();
