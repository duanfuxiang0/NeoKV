...
public:
    ...
    // 注册 Redis 索引信息 (不需要 SQL 表)
    // 这允许 SplitCompactionFilter 正确处理 Redis keys
    void register_redis_index(int64_t redis_index_id) {
        _split_index_map.modify([redis_index_id](butil::FlatMap<int64_t, IndexInfo*>& map) {
            if (map.seek(redis_index_id) == nullptr) {
                IndexInfo* redis_info = new IndexInfo;
                redis_info->id = redis_index_id;
                redis_info->type = pb::I_PRIMARY;  // Redis key 作为 primary key
                redis_info->is_global = false;
                redis_info->pk = redis_index_id;   // 自己是自己的 pk
                redis_info->name = "__redis__.kv.PRIMARY";
                redis_info->short_name = "PRIMARY";
                redis_info->length = -1;  // 变长
                redis_info->state = pb::IS_PUBLIC;
                map[redis_index_id] = redis_info;
                DB_WARNING("Registered Redis index: index_id=%ld", redis_index_id);
            }
        });
    }
