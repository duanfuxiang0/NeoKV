...
#define DB_DEBUG(_fmt_, args...)                                                                                       \
	do {                                                                                                               \
		com_writelog("DEBUG", "[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__,            \
		             bthread_self(), ##args);                                                                          \
	} while (0)
#else
#define DB_DEBUG(_fmt_, args...)
...
#define DB_TRACE(_fmt_, args...)                                                                                       \
	do {                                                                                                               \
		com_writelog("TRACE", "[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__,            \
		             bthread_self(), ##args);                                                                          \
	} while (0)

#define DB_NOTICE(_fmt_, args...)                                                                                      \
	do {                                                                                                               \
		com_writelog("NOTICE", "[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__,           \
		             bthread_self(), ##args);                                                                          \
	} while (0)
...
#define DB_WARNING(_fmt_, args...)                                                                                     \
	do {                                                                                                               \
		com_writelog("WARNING", "[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__,          \
		             bthread_self(), ##args);                                                                          \
	} while (0)

#define DB_FATAL(_fmt_, args...)                                                                                       \
	do {                                                                                                               \
		com_writelog("FATAL", "[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__,            \
		             bthread_self(), ##args);                                                                          \
	} while (0)
...
#define SQL_TRACE(_fmt_, args...)                                                                                      \
	do {                                                                                                               \
		com_writelog("MY_TRACE", "[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__,         \
		             bthread_self(), ##args);                                                                          \
	} while (0)
...
public:
	const char* LogSeverityNames[4] = {"INFO", "WARNING", "ERROR", "FATAL"};
...
#define DB_NOTICE_LONG(_fmt_, args...)                                                                                 \
	do {                                                                                                               \
		::neokv::glog_info_writelog_long("[%s:%d][%s][%llu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__,              \
...
#define DB_DEBUG(_fmt_, args...)                                                                                       \
	do {                                                                                                               \
		if (!FLAGS_enable_debug)                                                                                       \
			break;                                                                                                     \
		::neokv::glog_info_writelog("[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__,      \
		                            bthread_self(), ##args);                                                           \
	} while (0);
#else
#define DB_DEBUG(_fmt_, args...)
...
#define DB_WARNING(_fmt_, args...)                                                                                     \
	do {                                                                                                               \
		::neokv::glog_warning_writelog("[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__,   \
		                               bthread_self(), ##args);                                                        \
	} while (0);

#define DB_FATAL(_fmt_, args...)                                                                                       \
	do {                                                                                                               \
		::neokv::glog_error_writelog("[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__,     \
		                             bthread_self(), ##args);                                                          \
	} while (0);
...
#define SQL_TRACE(_fmt_, args...)                                                                                      \
	do {                                                                                                               \
		if (!FLAGS_enable_self_trace)                                                                                  \
			break;                                                                                                     \
		::neokv::glog_info_writelog_long("[%s:%d][%s][%lu]" _fmt_, strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__, \
		                                 bthread_self(), ##args);                                                      \
	} while (0);
...
#define DB_DEBUG_CLIENT(sock, _fmt_, args...)                                                                          \
	do {                                                                                                               \
		DB_DEBUG("user=%s fd=%d ip=%s port=%d errno=%d:" _fmt_, sock->username.c_str(), sock->fd, sock->ip.c_str(),    \
		         sock->port, sock->query_ctx->stat_info.error_code, ##args);                                           \
	} while (0);

#define DB_TRACE_CLIENT(sock, _fmt_, args...)                                                                          \
	do {                                                                                                               \
		DB_TRACE("user=%s fd=%d ip=%s port=%d errno=%d:" _fmt_, sock->username.c_str(), sock->fd, sock->ip.c_str(),    \
		         sock->port, sock->query_ctx->stat_info.error_code, ##args);                                           \
	} while (0);

#define DB_NOTICE_CLIENT(sock, _fmt_, args...)                                                                         \
	do {                                                                                                               \
		DB_NOTICE("user=%s fd=%d ip=%s port=%d errno=%d:" _fmt_, sock->username.c_str(), sock->fd, sock->ip.c_str(),   \
		          sock->port, sock->query_ctx->stat_info.error_code, ##args);                                          \
	} while (0);

#define DB_WARNING_CLIENT(sock, _fmt_, args...)                                                                        \
	do {                                                                                                               \
		DB_WARNING("user=%s fd=%d ip=%s port=%d errno=%d log_id=%lu:" _fmt_, sock->username.c_str(), sock->fd,         \
		           sock->ip.c_str(), sock->port, sock->query_ctx->stat_info.error_code,                                \
		           sock->query_ctx->stat_info.log_id, ##args);                                                         \
	} while (0);

#define DB_FATAL_CLIENT(sock, _fmt_, args...)                                                                          \
	do {                                                                                                               \
		DB_FATAL("user=%s fd=%d ip=%s port=%d errno=%d:" _fmt_, sock->username.c_str(), sock->fd, sock->ip.c_str(),    \
		         sock->port, sock->query_ctx->stat_info.error_code, ##args);                                           \
	} while (0);

#define DB_WARNING_STATE(state, _fmt_, args...)                                                                        \
	do {                                                                                                               \
		DB_WARNING("log_id: %lu, region_id: %ld, table_id: %ld," _fmt_, state->log_id(), state->region_id(),           \
		           state->table_id(), ##args);                                                                         \
	} while (0);
#define DB_FATAL_STATE(state, _fmt_, args...)                                                                          \
	do {                                                                                                               \
		DB_FATAL("log_id: %lu, region_id: %ld, table_id: %ld," _fmt_, state->log_id(), state->region_id(),             \
		         state->table_id(), ##args);                                                                           \
	} while (0);
