...
public:
	...
	int send_request_internal(const std::string& service_name, const Request& request, Response& response,
		...
		do {
			...
			if (response.errcode() == pb::HAVE_NOT_INIT) {
				DB_WARNING("connect with server fail. HAVE_NOT_INIT  log_id:%lu", cntl.log_id());
				set_leader_address(meta_interact_info, butil::EndPoint());
				continue;
			}
			if (response.errcode() == pb::NOT_LEADER) {
				DB_WARNING("connect with meta server:%s fail. not leader, redirect to :%s, log_id:%lu",
				           butil::endpoint2str(cntl.remote_side()).c_str(), response.leader().c_str(), cntl.log_id());
			...
			if (response.errcode() != pb::SUCCESS) {
				DB_WARNING("send meta server fail, log_id:%lu, response:%s", cntl.log_id(),
				           response.ShortDebugString().c_str());
				return -1;
			} else {
				return 0;
	...
	template <typename Request, typename Response>
	int send_other_meta_request(const std::string& service_name, const Request& request, Response& response,
		...
		if (ret != 0) {
			DB_WARNING("Fail to del_meta_info, meta_id: %ld, request: %s", meta_id, request.ShortDebugString().c_str());
			return -1;
		}
		ret = send_request_internal(service_name, request, response, *meta_interact_info);
		if (ret != 0) {
			DB_WARNING("Fail to send_request_internal, meta_id: %ld, request: %s", meta_id,
			           request.ShortDebugString().c_str());
			return -1;
		}
		ret = add_meta_info(response, meta_id);
		if (ret != 0) {
			DB_WARNING("Fail to add_meta_info, meta_id: %ld, response: %s", meta_id,
			           response.ShortDebugString().c_str());
			return -1;
		}
		return 0;
