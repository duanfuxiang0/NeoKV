...
DECLARE_int32(follow_read_timeout_s);
DECLARE_string(raftlog_uri);
DECLARE_string(stable_uri);
DECLARE_string(snapshot_uri);
DECLARE_bool(demotion_read_index_without_leader);
DECLARE_bool(use_read_index);
DECLARE_int32(redis_port);
DECLARE_bool(redis_cluster_enabled);
DECLARE_string(redis_advertise_ip);
...
enum SerializeStatus { STMPS_SUCCESS, STMPS_FAIL, STMPS_NEED_RESIZE };

enum SstBackupType { UNKNOWN_BACKUP, META_BACKUP, DATA_BACKUP };
...
public:
	...
	ConcurrencyBthread(int concurrency, const bthread_attr_t* attr) : _concurrency(concurrency), _attr(attr) {
	}
	void run(const std::function<void()>& call) {
...
public:
	...
	void run(int timeout_ms, const std::function<void()>& call) {
		...
		*_call = call;
		bthread_timer_add(
		    &_timer, butil::milliseconds_from_now(timeout_ms),
		    [](void* p) {
			    bthread_t tid;
...
public:
	BthreadLocal() : _bthread_local_key(INVALID_BTHREAD_KEY) {
		bthread_key_create(&_bthread_local_key, [](void* data) { delete static_cast<T*>(data); });
	}
	~BthreadLocal() {
...
#define ON_SCOPE_EXIT(callback)             neokv::ScopeGuard SCOPEGUARD_LINENAME(scope_guard, __LINE__)(callback)
#ifndef SAFE_DELETE
...
public:
	...
	const VALUE get_or_put_call(const KEY& key, const std::function<bool(VALUE& value)>& call1,
	...
	void traverse_copy(const std::function<void(VALUE& value)>& call1, const std::function<void(VALUE& value)>& call2) {
		for (uint32_t i = 0; i < MAP_COUNT; i++) {
			std::unordered_map<KEY, VALUE> tmp;
			{
				BAIDU_SCOPED_LOCK(_mutex[i]);
	...
	bool traverse_with_early_return_by_order(const std::function<bool(VALUE& value)>& call) {
		...
		std::sort(keys.begin(), keys.end(), [](const std::pair<KEY, uint32_t>& a, const std::pair<KEY, uint32_t>& b) {
...
public:
	...
	void put_incremental_info(const int64_t apply_index, T& infos) {
		...
		auto call_func = [=, this](DoubleBuffer<std::map<int64_t, T>>& buf) -> int {
			...
			(*background)[apply_index] = infos;
			// 当bg中最早的元素大于回收时间时清理fg，互换bg和fg；这样可以保证清理掉的都是大于超时时间的，极端情况下超时回收时间变为2倍的gc
			// time
	...
	bool check_and_update_incremental(std::function<void(const T&)> update_incremental, int64_t& last_updated_index,
		...
		if (frontground->size() == 0 && background->size() == 0) {
			...
			if (last_updated_index + 1 < background->begin()->first) {
				DB_WARNING("first:%ld, background->size = %lu, frontground size = 0", background->begin()->first,
				           background->size());
				return true;
			} else {
			...
			if (last_updated_index + 1 < frontground->begin()->first) {
				DB_WARNING("first:%ld, frontground size = %lu, background size = %lu", frontground->begin()->first,
				           frontground->size(), background->size());
				return true;
			} else {
...
public:
	...
	SlowQueryInfo(uint64_t log_id_, uint64_t sign_, int64_t start_time_, int64_t end_time_, int64_t exec_time_,
	              int64_t filtered_rows_, int64_t affected_rows_, int64_t scan_rows_, int64_t return_rows_,
	              const std::string& ip_, const std::string& resource_tag_, const std::string& user_name_,
	              const std::string& family_, const std::string& table_name_, const std::string& sql_, bool status_)
	    : log_id(log_id_), sign(sign_), start_time(start_time_), end_time(end_time_), exec_time(exec_time_),
	      filtered_rows(filtered_rows_), affected_rows(affected_rows_), scan_rows(scan_rows_),
	      return_rows(return_rows_), ip(ip_), resource_tag(resource_tag_), user_name(user_name_), family(family_),
...
public:
	...
	BvarSlowQueryMap& operator+=(const BvarSlowQueryMap& other) {
		...
		if (other.internal_slow_query_map.size() == 1 &&
		    internal_slow_query_map[other_slow_query_map.begin()->first].size() > FLAGS_limit_slow_sql_size) {
			return *this;
		}
...
struct BvarMap {
	struct SumCount {
		...
		SumCount(int64_t table_id, int64_t sum, int64_t err_sum, int64_t count, int64_t err_count,
		         int64_t affected_rows, int64_t scan_rows, int64_t read_disk_size, int64_t filter_rows,
		         int64_t region_count, const std::map<int32_t, int>& field_range_type_, const uint64_t sign_,
		         const std::set<uint64_t>& subquery_signs_)
		    : table_id(table_id), sum(sum), err_sum(err_sum), count(count), err_count(err_count),
		      affected_rows(affected_rows), scan_rows(scan_rows), read_disk_size(read_disk_size),
		      filter_rows(filter_rows), region_count(region_count) {
...
public:
	...
	BvarMap(const std::string& key, int64_t index_id, int64_t table_id, int64_t cost, int64_t err_cost,
	        int64_t affected_rows, int64_t scan_rows, int64_t read_disk_size, int64_t filter_rows, int64_t region_count,
	        const std::map<int32_t, int>& field_range_type_, int64_t err_count, uint64_t parent_sign,
	        std::set<uint64_t>& subquery_signs) {
		internal_map[key][index_id] =
		    SumCount(table_id, cost, err_cost, 1, err_count, affected_rows, scan_rows, read_disk_size, filter_rows,
		             region_count, field_range_type_, parent_sign, subquery_signs);
	}
...
private:
	RocksdbVars()
	    : rocksdb_put_time_cost_latency("rocksdb_put_time_cost_latency", &rocksdb_put_time, -1),
...
private:
	void shiftdown(size_t index) {
		...
		if (left_index < _heap.size() && Compare()(_heap[left_index], _heap[min_index])) {
			min_index = left_index;
		}
		if (right_index < _heap.size() && Compare()(_heap[right_index], _heap[min_index])) {
...
extern int get_instance_from_bns(int* ret, const std::string& bns_name, std::vector<std::string>& instances,
                                 bool need_alive = true, bool white_list = false);
extern int get_multi_port_from_bns(int* ret, const std::string& bns_name, std::vector<std::string>& instances,
...
struct IconvEncoding {
	...
	inline static constexpr int32_t get_character_bytes() {
		return To == pb::GBK ? 2 : To == pb::UTF8 ? 3 : -1;
	}
...
extern int convert_charset(const pb::Charset& from_charset, const std::string& from_str, const pb::Charset& to_charset,
                           std::string& to_str);
