...
struct ConvertToSyncClosure : public braft::Closure {
	ConvertToSyncClosure(BthreadCond& _sync_sign, int64_t _region_id) : sync_sign(_sync_sign), region_id(_region_id) {};
	virtual void Run();
...
struct SnapshotClosure : public braft::Closure {
	virtual void Run() {
		if (!status().ok()) {
			DB_WARNING("region_id: %ld  status:%s, snapshot failed.", region->get_region_id(), status().error_cstr());
		}
		// 遇到部分请求报has no applied logs since last snapshot
		// 不调用on_snapshot_save导致不更新_snapshot_time_cost等信息
		if (region != nullptr) {
			// Neo-redis: vector_truncate_del() removed, no vector index
