...
int main(int argc, char** argv) {
	...
	if (neokv::FLAGS_meta_server_bns.find(":") != std::string::npos) {
		...
		} while (ret != webfoot::WEBFOOT_RET_SUCCESS && ret != webfoot::WEBFOOT_SERVICE_NOTEXIST &&
	...
	DB_WARNING("completely deploy:%d, host:%s", completely_deploy, butil::endpoint2str(addr).c_str());
#else
	// 指定的是ip:port的形式
	std::vector<std::string> list_raft_peers;
	boost::split(list_raft_peers, neokv::FLAGS_meta_server_bns, boost::is_any_of(","));
