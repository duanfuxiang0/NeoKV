...
void SchemaManager::process_schema_info(google::protobuf::RpcController* controller,
    ...
    default:
        ERROR_SET_RESPONSE(response, pb::UNSUPPORT_REQ_TYPE,
                           "neo-redis meta only supports region split/merge/update/drop",
...
int SchemaManager::pre_process_for_merge_region(const pb::MetaManagerRequest* request,
    ...
    if (best == nullptr) {
        ERROR_SET_RESPONSE_WARN(response, pb::INPUT_PARAM_ERROR, "dst merge region not exist", request->op_type(), log_id);
        return -1;
    }
    ...
    response->set_op_type(request->op_type());
    DB_WARNING("find dst merge region instance:%s, table_id:%ld, src_region_id:%ld, dst_region_id:%ld, time_cost:%ld, log_id:%lu",
               out->dst_instance().c_str(), table_id, src_region_id, out->dst_region_id(), time_cost.get_time(), log_id);
    return 0;
}
...
int SchemaManager::pre_process_for_split_region(const pb::MetaManagerRequest* request,
    ...
    if (parent_region->peers_size() < 2 && select_replica_num > 1) {
        ERROR_SET_RESPONSE_WARN(response, pb::INPUT_PARAM_ERROR, "region not stable, cannot split",
                                request->op_type(), log_id);
        return -1;
    }
