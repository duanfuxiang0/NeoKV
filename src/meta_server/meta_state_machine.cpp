...
void MetaStateMachine::store_heartbeat(google::protobuf::RpcController* controller,
	...
	DB_NOTICE("store:%s heart beat, time_cost:%ld, instance_time:%ld, peer_balance_time:%ld, peer_time:%ld, "
	          ...
	          request->instance_info().address().c_str(), time_cost.get_time(), instance_time, peer_balance_time,
	          peer_time, leader_region_time, leader_status_time, log_id);
}

void MetaStateMachine::neo_heartbeat(google::protobuf::RpcController* controller,
                                     const pb::NeoHeartBeatRequest* request, pb::NeoHeartBeatResponse* response,
                                     google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	brpc::Controller* cntl = static_cast<brpc::Controller*>(controller);
	uint64_t log_id = 0;
...
bool MetaStateMachine::whether_can_decide() {
	...
	return butil::gettimeofday_us() - _leader_start_timestmap > (int64_t)3 * FLAGS_store_heart_beat_interval_us;
}
...
void MetaStateMachine::on_apply(braft::Iterator& iter) {
	for (; iter.valid(); iter.next()) {
		...
		DB_NOTICE("on apply, term:%ld, index:%ld, request op_type:%s", iter.term(), iter.index(),
...
void MetaStateMachine::on_snapshot_save(braft::SnapshotWriter* writer, braft::Closure* done) {
	...
	auto iter =
	    RocksWrapper::get_instance()->new_iterator(read_options, RocksWrapper::get_instance()->get_meta_info_handle());
	iter->SeekToFirst();
	Bthread bth(&BTHREAD_ATTR_SMALL);
	bth.run([this, done, iter, writer]() { save_snapshot(done, iter, writer); });
}

void MetaStateMachine::save_snapshot(braft::Closure* done, rocksdb::Iterator* iter, braft::SnapshotWriter* writer) {
	brpc::ClosureGuard done_guard(done);
	...
	rocksdb::Options option =
	    RocksWrapper::get_instance()->get_options(RocksWrapper::get_instance()->get_meta_info_handle());
...
int MetaStateMachine::on_snapshot_load(braft::SnapshotReader* reader) {
	...
	auto status =
	    RocksWrapper::get_instance()->remove_range(options, RocksWrapper::get_instance()->get_meta_info_handle(),
	                                               remove_start_key, MetaServer::MAX_IDENTIFY, false);
	if (!status.ok()) {
	...
	for (auto& file : files) {
		...
		// Load max_region_id.
		{
			...
			std::string value;
			auto s = RocksWrapper::get_instance()->get(
			    rocksdb::ReadOptions(), RocksWrapper::get_instance()->get_meta_info_handle(), key, &value);
		...
		// Load all regions.
		{
			...
			std::unique_ptr<rocksdb::Iterator> it(
			    RocksWrapper::get_instance()->new_iterator(ro, RocksWrapper::get_instance()->get_meta_info_handle()));
