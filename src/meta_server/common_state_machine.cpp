...
void MetaServerClosure::Run() {
	if (!status().ok()) {
		...
		DB_FATAL("meta server closure fail, error_code:%d, error_mas:%s", status().error_code(), status().error_cstr());
	}
	...
	if (response != nullptr && response->op_type() != pb::OP_GEN_ID_FOR_AUTO_INCREMENT) {
		DB_NOTICE("request:%s, response:%s, raft_time_cost:[%ld], total_time_cost:[%ld], remote_side:[%s]",
		          request.c_str(), response->ShortDebugString().c_str(), raft_time_cost, total_time_cost,
		          remote_side.c_str());
	}
...
void CommonStateMachine::process(google::protobuf::RpcController* controller, const pb::MetaManagerRequest* request,
...
void CommonStateMachine::on_leader_stop(const butil::Status& status) {
	DB_WARNING("leader stop, error_code:%d, error_des:%s", status.error_code(), status.error_cstr());
	on_leader_stop();
}
void CommonStateMachine::on_error(const ::braft::Error& e) {
	DB_FATAL("meta state machine error, error_type:%d, error_code:%d, error_des:%s", e.type(), e.status().error_code(),
	         e.status().error_cstr());
}
...
void CommonStateMachine::check_migrate() {
	...
	if (get_instance_from_bns(&ret, FLAGS_meta_server_bns, instances, false) != 0 ||
		...
		DB_WARNING("get instance from bns fail, bns:%s, ret:%d, instance.size:%lu", FLAGS_meta_server_bns.c_str(), ret,
		           instances.size());
		return;
	}
