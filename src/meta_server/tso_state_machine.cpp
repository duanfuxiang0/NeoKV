...
void TSOStateMachine::process(google::protobuf::RpcController* controller, const pb::TsoRequest* request,
...
void TSOStateMachine::reset_tso(const pb::TsoRequest& request, braft::Closure* done) {
	if (request.has_current_timestamp() && request.has_save_physical()) {
		...
		if (physical < _tso_obj.last_save_physical || current.physical() < _tso_obj.current_timestamp.physical()) {
			if (!request.force()) {
				DB_WARNING("time fallback save_physical:(%ld, %ld) current:(%ld, %ld, %ld, %ld)", physical,
				           _tso_obj.last_save_physical, current.physical(), _tso_obj.current_timestamp.physical(),
...
int TSOStateMachine::sync_timestamp(const pb::TsoTimestamp& current_timestamp, int64_t save_physical) {
	...
	if (response.errcode() != pb::SUCCESS) {
		DB_FATAL("sync timestamp failed, request:%s response:%s", request.ShortDebugString().c_str(),
...
void TSOStateMachine::on_leader_start() {
	...
	auto func = [this, last_save, current]() {
		DB_WARNING("leader_start current(phy:%ld,log:%ld) save:%ld", current.physical(), current.logical(), last_save);
...
void TSOStateMachine::save_snapshot(braft::Closure* done, braft::SnapshotWriter* writer, std::string sto_str) {
	brpc::ClosureGuard done_guard(done);
...
int TSOStateMachine::load_tso(const std::string& tso_file) {
	...
	std::string extra((std::istreambuf_iterator<char>(extra_fs)), std::istreambuf_iterator<char>());
	try {
