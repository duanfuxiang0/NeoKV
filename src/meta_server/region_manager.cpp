...
void RegionManager::update_region(const pb::MetaManagerRequest& request, const int64_t apply_index,
                                  braft::Closure* done) {
	TimeCost time_cost;
	...
	if (request.has_region_info()) {
		DB_WARNING("use optional region_info region_id:%ld", request.region_info().region_id());
	...
	for (auto& region_info : region_infos) {
		...
		if (ret < 0) {
			DB_WARNING("table name: %s / partition_id: %ld not exist, region_info:%s", region_info.table_name().c_str(),
			           partition_id, region_info.ShortDebugString().c_str());
		...
		if (master_region_ptr != nullptr) {
			if (region_info.version() < master_region_ptr->version()) {
				DB_WARNING("region_id: %ld, request version %ld < master version %ld", region_id, region_info.version(),
				           master_region_ptr->version());
			...
			if (region_info.version() == master_region_ptr->version() &&
				...
				if (peer_is_equal(region_info, *master_region_ptr)) {
					DB_WARNING("region_id: %ld, request conf_version  %ld <= master conf_version %ld", region_id,
					           region_info.conf_version(), master_region_ptr->conf_version());
		...
		if (!key_init) {
			...
			if (g_table_id != table_id) {
				DB_FATAL("two region has different table id %ld vs %ld", g_table_id, table_id);
				return;
			}
		}
	...
	if (!old_pb && !add_delete_region) {
		// 兼容旧的pb，old_pb不检查区间
		bool check_ok = TableManager::get_instance()->check_region_when_update(g_table_id, min_start_key, max_end_key);
		if (!check_ok) {
	...
	if (old_pb || add_delete_region) {
		...
		TableManager::get_instance()->update_startkey_regionid_map_old_pb(g_table_id, key_id_map);
	} else {
		TableManager::get_instance()->update_startkey_regionid_map(g_table_id, min_start_key, max_end_key, key_id_map);
	}
	// 更新内存值
	int i = 0;
	for (auto& region_info : region_infos) {
	...
	DB_NOTICE("update region success, request:%s, time_cost:%ld", request.ShortDebugString().c_str(),
	          time_cost.get_time());
}
// 根据region_id恢复store上误删除的region
// 如果待回复的reigon信息存在则直接新建，如果不存在，则根据前后region恢复
void RegionManager::restore_region(const pb::MetaManagerRequest& request, pb::MetaManagerResponse* response) {
	...
	*(init_region_request.mutable_region_info()) = region_info;
	...
	if (ret < 0) {
		DB_FATAL("create table fail, address:%s, region_id: %ld", init_region_request.region_info().leader().c_str(),
		         region_id);
	...
	DB_NOTICE("new region_id: %ld success, table_name:%s", region_id, region_info.table_name().c_str());
}
// 删除region_id的操作只会在表已经删除或创建失败的情况下才会调用
// 所以在删除region时表信息已经不存在，不在需要更新表信息
void RegionManager::drop_region(const pb::MetaManagerRequest& request, const int64_t apply_index,
                                braft::Closure* done) {
	...
	if (ret < 0) {
		DB_WARNING("drop region fail, region_info: %s", request.ShortDebugString().c_str());
		IF_DONE_SET_RESPONSE(done, pb::INTERNAL_ERROR, "write db fail");
		return;
	}
	// 删除内存中的值
	...
	erase_region_info(drop_region_ids, result_region_ids, result_partition_ids, result_table_ids, result_start_keys,
	                  result_end_keys);
	erase_region_state(drop_region_ids);
	std::vector<pb::RegionInfo> region_infos;
	for (uint32_t i = 0; i < result_region_ids.size() && i < result_table_ids.size() && i < result_start_keys.size() &&
	                     i < result_end_keys.size();
	     i++) {
		pb::RegionInfo region_info;
...
void RegionManager::split_region(const pb::MetaManagerRequest& request, braft::Closure* done) {
	...
	if (ret != 0) {
		DB_WARNING("add max_region_id to rocksdb fail when split region:%s", request.ShortDebugString().c_str());
		IF_DONE_SET_RESPONSE(done, pb::INTERNAL_ERROR, "write db fail");
		return;
	}
	...
	if (done && ((MetaServerClosure*)done)->response) {
		...
		if (new_region_num == 1) {
			...
			for (int64_t i = new_region_start_id; i <= new_region_end_id; ++i) {
				...
				((MetaServerClosure*)done)
				    ->response->mutable_split_response()
				    ->mutable_multi_new_regions(pos)
				    ->set_new_region_id(i);
			}
		}
		...
		for (auto& peer : peers) {
			auto drop_region_fun = [&concurrency_cond, log_id, peer, drop_region_id] {
				...
				DB_WARNING("send remove region request:%s, response:%s, peer_address:%s, region_id:%ld",
				           request.ShortDebugString().c_str(), response.ShortDebugString().c_str(), peer.c_str(),
				           drop_region_id);
			};
			Bthread bth;
...
// default for MIGRATE
void RegionManager::add_peer_for_store(const std::string& instance, InstanceStateInfo status) {
	...
	DB_WARNING("add peer all region for migrate store start, store:%s, idc: %s", instance.c_str(),
	           instance_idc.to_string().c_str());
	...
	if (status.state_duration.get_time() < 10 * 60 * 1000 * 1000LL && instance_count > 3 && leader_count > 10 &&
			...
			auto add_peer_fun_per_instance = [this, add_peer_per_instance]() {
				...
				for (auto& add_peer_request : add_peer_per_instance.second) {
					auto add_peer_fun = [this, leader, add_peer_request]() {
						...
						DB_WARNING("send add peer leader: %s, request:%s, response:%s, ret: %d", leader.c_str(),
						           add_peer_request.ShortDebugString().c_str(), response.ShortDebugString().c_str(),
						           ret);
...
void RegionManager::add_all_learner_for_store(const std::string& instance, const IdcInfo& idc,
	...
	for (auto& region_id : learner_ids) {
		...
		ClusterManager::get_instance()->get_resource_tag_count(ptr_region->learners(), resource_tag,
	...
	for (auto request : add_learner_requests) {
		auto add_learner_peer_fun = [this, request]() {
			...
			DB_WARNING("send %s add learn request:%s, response:%s, ret: %d", request.first.c_str(),
			           request.second.ShortDebugString().c_str(), response.ShortDebugString().c_str(), ret);
			if (ret != 0) {
...
// default for DEAD
void RegionManager::delete_all_region_for_store(const std::string& instance, InstanceStateInfo status) {
	IdcInfo instance_idc;
	if (ClusterManager::get_instance()->get_instance_idc(instance, instance_idc) < 0) {
		DB_FATAL("instance: %s get idc fail", instance.c_str());
		return;
	}
	DB_WARNING("delete all region for dead store start, dead_store:%s, idc:%s", instance.c_str(),
	           instance_idc.to_string().c_str());
	std::vector<int64_t> learner_ids;
	get_learner_ids(instance, learner_ids);
	if (learner_ids.size() > 0) {
	...
	for (auto request : requests) {
		auto remove_peer_fun = [this, request]() {
			...
			DB_WARNING("send remove peer request:%s, response:%s", request.ShortDebugString().c_str(),
...
void RegionManager::pre_process_remove_peer_for_store(const std::string& instance, const IdcInfo& instance_idc,
                                                      pb::Status status,
                                                      std::vector<pb::RaftControlRequest>& requests) {
	std::vector<int64_t> region_ids;
	get_region_ids(instance, region_ids);
	for (auto& region_id : region_ids) {
		...
		if (ret < 0 || ptr_region->peers_size() < replica_num) {
			...
			// 故障需要尽快恢复，轮询最均匀
			auto ret = ClusterManager::get_instance()->select_instance_rolling(new_peer_idc, peers, new_instance);
			if (ret < 0) {
			...
			auto add_peer_fun = [add_peer, leader]() {
				StoreInteract store_interact(leader.c_str());
				pb::StoreRes response;
				auto ret = store_interact.send_request("add_peer", add_peer, response);
				DB_WARNING("send add peer leader: %s, request:%s, response:%s, ret: %d", leader.c_str(),
				           add_peer.ShortDebugString().c_str(), response.ShortDebugString().c_str(), ret);
			};
		...
		if (ret < 0 || status != pb::NORMAL) {
			DB_WARNING("region_id:%ld status:%s is not normal, can not been remove, instance:%s", region_id,
			           pb::Status_Name(status).c_str(), instance.c_str());
			continue;
		}
...
void RegionManager::pre_process_add_peer_for_store(
    const std::string& instance, const IdcInfo& instance_idc, pb::Status status,
    std::unordered_map<std::string, std::vector<pb::AddPeer>>& add_peer_requests) {
	std::vector<int64_t> region_ids;
	get_region_ids(instance, region_ids);
	for (auto& region_id : region_ids) {
		...
		auto ret = TableManager::get_instance()->get_replica_num(table_id, replica_num);
		if (ret < 0 || ptr_region->peers_size() > replica_num) {
			DB_WARNING("region_id: %ld has been added peer, region_info: %s", region_id,
			           ptr_region->ShortDebugString().c_str());
			continue;
		}
		...
		// 故障需要尽快恢复，轮询最均匀
		ret = ClusterManager::get_instance()->select_instance_rolling(new_peer_idc, peers, new_instance);
		if (ret < 0) {
			DB_FATAL("select store from cluster fail, region_id:%ld, idc: %s", region_id,
			         new_peer_idc.to_string().c_str());
			continue;
...
bool RegionManager::binlog_peer_can_delete(const std::string& instance, int64_t region_id, int64_t table_id) {
	...
	if (table_binlog_max_keep_time > 0 && max_time_interval >= table_binlog_max_keep_time * 0.8) {
		return true;
	}
...
bool RegionManager::check_binlog_regions_can_migrate(const std::string& instance) {
	...
	DB_WARNING("checked store: %s all binlog regions can migrate success, binlog region cnt: %lu", instance.c_str(),
	           region_ids.size());
	return true;
}
...
void RegionManager::leader_main_logical_room_check(const pb::StoreHeartBeatRequest* request,
                                                   pb::StoreHeartBeatResponse* response, IdcInfo& leader_idc,
	...
	for (auto& leader_region : request->leader_regions()) {
		...
		// leader在主机房直接跳过
		// DB_WARNING("instance: %s region_id:%ld do main_logical_room check leader_logical_room:%s
		// main_logical_room:%s",
		...
		for (auto& peer : region_info.peers()) {
			...
			if (st != pb::NORMAL) {
				...
				DB_WARNING("region_id:%ld instance: %s peer: %s not NORMAL not transfer leader", region_id,
				           instance.c_str(), peer.c_str());
				break;
			}
		...
		if (!candicate_instances.empty()) {
			...
			*(response->add_trans_leader()) = transfer_request;
			DB_WARNING("instance: %s region_id:%ld do leader transfer leader_idc: %s, main_idc:%s "
			           "transfer_request:%s",
			           instance.c_str(), region_id, leader_idc.to_string().c_str(), main_idc.to_string().c_str(),
 ...
 */
void RegionManager::leader_load_balance_on_pk_prefix(
    const std::string& instance, const pb::StoreHeartBeatRequest* request,
    ...
    const std::set<int64_t>& trans_leader_region_ids, std::unordered_map<int64_t, int64_t>& table_transfer_leader_count,
    std::unordered_map<std::string, int64_t>& pk_prefix_leader_count,
    std::unordered_map<int64_t, int64_t>& table_replica, std::unordered_map<int64_t, IdcInfo>& table_main_idc,
	...
	for (auto& table_pk_prefix_pair : pk_prefix_leader_region_map) {
		...
		for (auto& pk_prefix_region_pair : table_pk_prefix_pair.second) {
			...
			int64_t peer_count =
			    ClusterManager::get_instance()->get_pk_prefix_peer_count(pk_prefix_region_pair.first, IdcInfo());
			...
			DB_DEBUG("pk_prefix: %s, peer_count: %lu, replica_num: %lu, instance_count: %lu, leader_count: %lu, "
			         ...
			         pk_prefix_region_pair.first.c_str(), peer_count, replica_num, instance_count, leader_count,
			         average_leader_count, pk_prefix_trans_leader_count);
	...
	for (auto& leader_region : request->leader_regions()) {
		...
		if (!transfer_to_peer.empty()) {
			...
			*(response->add_trans_leader()) = transfer_request;
			...
			DB_WARNING("instance: %s region_id:%ld do leader transfer transfer_request:%s", instance.c_str(), region_id,
			           transfer_request.ShortDebugString().c_str());
		}
	}
...
void RegionManager::leader_load_balance(bool whether_can_decide, bool load_balance,
	...
	for (auto& leader_region : request->leader_regions()) {
		...
		std::string key;
		if (!TableManager::get_instance()->get_pk_prefix_key(table_id, table_pk_prefix_dimension[table_id],
		                                                     leader_region.region().start_key(), key)) {
			continue;
		}
	...
	if (!request->need_leader_balance() && instance_status != pb::MIGRATE && instance_status != pb::SLOW) {
		return;
	}
	std::string resource_tag = request->instance_info().resource_tag();
	if (!whether_can_decide) {
		DB_WARNING("meta state machine can not decide, resource_tag: %s, instance: %s", resource_tag.c_str(),
		           instance.c_str());
		return;
	}

	if (!load_balance) {
		DB_WARNING("meta state machine close leader load balance, resource_tag: %s, instance: %s", resource_tag.c_str(),
		           instance.c_str());
		return;
	}
	DB_WARNING("leader load balance, resource_tag: %s, instance: %s", resource_tag.c_str(), instance.c_str());
	...
	std::unordered_map<int64_t, int64_t> table_total_instance_counts;
	// 本集群所有逻辑机房的实例数
	int64_t total_instance_count = ClusterManager::get_instance()->get_instance_count(resource_tag, &room_count);
	for (auto& table_leader_count : table_leader_counts) {
		...
		if (instance_status == pb::SLOW) {
			...
			DB_WARNING("SLOW transfer lead for instance: %s, table_id: %ld, instance_count: %ld,"
			           ...
			           instance.c_str(), table_id, instance_count, average_leader_count, table_leader_count.second);
		}
		...
		if (table_leader_count.second > (average_leader_count + average_leader_count * 5 / 100)) {
			transfer_leader_count[table_id] = 2 * (table_leader_count.second - average_leader_count);
	...
	if (can_do_pk_prefix_balance && !pk_prefix_leader_count.empty() && instance_status != pb::MIGRATE &&
	    instance_status != pb::SLOW) {
		leader_load_balance_on_pk_prefix(instance, request, table_total_instance_counts, pk_prefix_leader_regions,
		                                 trans_leader_region_ids, transfer_leader_count, pk_prefix_leader_count,
		                                 table_replica, table_main_idc, response);
	}
	...
	for (auto& table_count : transfer_leader_count) {
		...
		DB_WARNING("transfer lead for instance: %s, table_id: %ld,"
		           ...
		           instance.c_str(), table_count.first, average_leader_counts[table_count.first], table_count.second);
	}
...
void RegionManager::pk_prefix_load_balance(const std::unordered_map<std::string, int64_t>& pk_prefix_add_peer_counts,
	...
	for (auto& add_peer_count : pk_prefix_add_peer_counts) {
		...
		for (size_t i = 0; i < total_region_count; ++i, ++index) {
			...
			std::string new_instance;
			ret = ClusterManager::get_instance()->select_instance_min_on_pk_prefix(
			    table_balance_idc[table_id], exclude_stores, table_id, table_pk_prefix_key, new_instance,
			    pk_prefix_average_counts[table_pk_prefix_key], table_average_counts[table_id]);
			if (ret < 0) {
				continue;
			}
			pb::AddPeer add_peer;
	...
	Bthread bth(&BTHREAD_ATTR_SMALL);
	auto add_peer_fun = [this, add_peer_requests, instance, region_idc]() {
		for (auto request : add_peer_requests) {
			...
			DB_WARNING("instance: %s pk_prefix peer load balance, send add peer leader: %s, "
			           ...
			           instance.c_str(), request.first.c_str(), request.second.ShortDebugString().c_str(),
...
void RegionManager::peer_load_balance(const std::unordered_map<int64_t, int64_t>& add_peer_counts,
	...
	for (auto& add_peer_count : add_peer_counts) {
		...
		for (size_t i = 0; i < total_region_count; ++i, ++index) {
			...
			*/
			...
			std::string new_instance;
			if (can_pk_prefix_can_decide && table_pk_prefix_dimension.count(table_id) > 0 &&
			    table_pk_prefix_dimension[table_id] > 0) {
				...
				if (!TableManager::get_instance()->get_pk_prefix_key(master_region_info->table_id(),
				                                                     ...
				                                                     master_region_info->start_key(), key)) {
					DB_WARNING("decode pk_prefix_key fail, table_id: %lu, region_id: %lu", table_id,
					           master_region_info->region_id());
					continue;
				}
				ret = ClusterManager::get_instance()->select_instance_min_on_pk_prefix(
				    balance_idc, exclude_stores, table_id, key, new_instance, pk_prefix_average_counts[key],
				    table_average_counts[table_id], true);
			} else {
				ret = ClusterManager::get_instance()->select_instance_min(balance_idc, exclude_stores, table_id,
				                                                          new_instance, table_average_counts[table_id]);
			}
			if (ret < 0) {
				continue;
			}
	...
	Bthread bth(&BTHREAD_ATTR_SMALL);
	auto add_peer_fun = [add_peer_requests, instance]() {
		for (auto request : add_peer_requests) {
			StoreInteract store_interact(request.first.c_str());
			pb::StoreRes response;
			auto ret = store_interact.send_request("add_peer", request.second, response);
...
void RegionManager::learner_load_balance(const std::unordered_map<int64_t, int64_t>& add_peer_counts,
                                         std::unordered_map<int64_t, std::vector<int64_t>>& instance_regions,
                                         const std::string& instance, const std::string& resource_tag,
                                         std::unordered_map<int64_t, int64_t>& table_average_counts) {
	std::vector<std::pair<std::string, pb::InitRegion>> add_learner_requests;
	for (auto& add_peer_count : add_peer_counts) {
		int64_t table_id = add_peer_count.first;
		...
		for (size_t i = 0; i < total_region_count; ++i, ++index) {
			...
			ClusterManager::get_instance()->get_resource_tag_count(master_region_info->learners(), resource_tag,
			                                                       current_instances);
			...
			std::string new_instance;
			ret = ClusterManager::get_instance()->select_instance_min({resource_tag, "", ""}, exclude_stores, table_id,
			                                                          new_instance, table_average_counts[table_id]);
			if (ret < 0) {
				continue;
			}
	...
	auto add_learn_fun = [this, add_learner_requests]() {
		for (auto request : add_learner_requests) {
			StoreInteract store_interact(request.first);
			pb::StoreRes response;
			auto ret = store_interact.send_request("init_region", request.second, response);
			DB_WARNING("send add learn request:%s, response:%s, ret: %d", request.second.ShortDebugString().c_str(),
			           response.ShortDebugString().c_str(), ret);
			if (ret != 0) {
				DB_WARNING("add learner node error.");
			}
...
void RegionManager::update_binlog_status(const pb::StoreHeartBeatRequest* request) {
	...
	for (auto& binlog_info : request->binlog_ts_infos()) {
		...
		_binlog_region_state_map.init_if_not_exist_else_update(
		    region_id, true, [table_id, region_id, instance, oldest_ts](BinlogRegionState& binlog_state) {
			    binlog_state.peer_oldest_timestamp_to_now_interval[instance] = oldest_ts;
			    binlog_state.table_id = table_id;
			    DB_DEBUG("table_id: %ld, region_id: %ld, instance: %s, set oldest ts: %ld", table_id, region_id,
			             instance.c_str(), oldest_ts);
		    });
	}
...
void RegionManager::update_leader_status(const pb::StoreHeartBeatRequest* request, int64_t timestamp) {
	for (auto& leader_region : request->leader_regions()) {
		...
		if (region_info.has_start_key() && region_info.has_end_key() && !region_info.start_key().empty() &&
		    !region_info.end_key().empty()) {
			if (region_info.start_key() == region_info.end_key()) {
		...
		if (leader_region.peers_status_size() > 0) {
			_region_peer_state_map.init_if_not_exist_else_update(
			    region_id, true, [&leader_region, timestamp](RegionPeerState& peer_state) {
				    ...
				    for (auto& peer_status : leader_region.peers_status()) {
					    ...
					    for (auto illegal_peer_iter = peer_state.ilegal_peers_state.begin();
					         illegal_peer_iter != peer_state.ilegal_peers_state.end(); ++illegal_peer_iter) {
...
bool RegionManager::check_and_update_incremental(
	...
	int cnt = 0;
	auto update_func = [response, need_heartbeat_table, &heartbeat_table_partition_map,
	                    &cnt](const std::vector<pb::RegionInfo>& region_infos) {
	...
	TimeCost cost;
	bool need_upd =
	    _incremental_region_info.check_and_update_incremental(update_func, last_updated_index, applied_index);
...
bool RegionManager::add_region_is_exist(int64_t table_id, const std::string& start_key, const std::string& end_key,
...
void RegionManager::leader_heartbeat_for_region(const pb::StoreHeartBeatRequest* request,
	...
	std::set<int64_t> binlog_table_ids;
	TableManager::get_instance()->get_table_info(related_table_ids, table_replica_nums, table_replica_dists_maps,
	...
	for (auto& leader_region : request->leader_regions()) {
		...
		if (master_region_info == nullptr) {
			...
			int ret = TableManager::get_instance()->whether_exist_table_partition(table_id, partition_id);
			if (ret < 0) {
				DB_WARNING("region_id: %ld, table_id: %ld, partition_id: %ld not exist", region_id, table_id,
				           partition_id);
				continue;
			}
			if (leader_region_info.start_key().empty() && leader_region_info.end_key().empty()) {
				...
				*(request.add_region_infos()) = leader_region_info;
				SchemaManager::get_instance()->process_schema_info(NULL, &request, NULL, NULL);
			} else if (true == add_region_is_exist(leader_region_info.table_id(), leader_region_info.start_key(),
			                                       leader_region_info.end_key(), leader_region_info.partition_id())) {
		...
		if (!peer_changed) {
			check_peer_count(region_id, leader_region, table_replica_nums, table_replica_dists_maps,
			                 remove_peer_requests, binlog_table_ids,
			                 table_pk_prefix_dimensions[leader_region_info.table_id()], response);
		}
		// learner
					...
					for (auto current_resource_learner : current_resource_learners) {
						...
						if (!is_health) {
							auto instance_status =
							    ClusterManager::get_instance()->get_instance_status(current_resource_learner);
							// 实例故障，等待实例迁移。
							if (instance_status == pb::FAULTY || instance_status == pb::DEAD ||
							    instance_status == pb::MIGRATE) {
								continue;
							} else {
								if (master_region_info->peers_size() > 1) {
									// region只有一个peer的时候暂缓加learner
	...
	auto remove_learn_fun = [this, remove_learner_requests]() {
		for (auto& remove_req : remove_learner_requests) {
			...
			DB_WARNING("send remove learn request:%s, response:%s, ret: %d",
			           remove_req.second.ShortDebugString().c_str(), response.ShortDebugString().c_str(), ret);
			if (ret != 0) {
	...
	Bthread bth(&BTHREAD_ATTR_SMALL);
	auto remove_peer_fun = [this, remove_peer_requests]() {
		for (auto request : remove_peer_requests) {
			...
			DB_WARNING("send remove peer request:%s, response:%s, ret: %d", request.second.ShortDebugString().c_str(),
			           response.ShortDebugString().c_str(), ret);
			if (ret == 0) {
				...
				DB_WARNING("send remove region to store:%s request: %s, resposne: %s, ret: %d", request.first.c_str(),
...
void RegionManager::check_whether_update_region(int64_t region_id, bool has_peer_changed, const std::string& leader,
	...
	if (leader_region_info.log_index() < master_region_info->log_index()) {
		DB_WARNING("leader: %s log_index:%ld in heart is less than in master:%ld, region_id: %ld",
		           leader_region_info.leader().c_str(), leader_region_info.log_index(), master_region_info->log_index(),
		           region_id);
		return;
	}
	...
	if (leader_region_info.version() <= master_region_info->version() && !leader_region.simple() &&
		...
		DB_FATAL("version not change, but start_key or end_key change,"
		         ...
		         master_region_info->ShortDebugString().c_str(), leader_region_info.ShortDebugString().c_str());
		return;
	}
	...
	// 前两种情况都是通过raft更新
	if (version_changed) {
		TableManager::get_instance()->check_update_region(leader_region, master_region_info);
		...
		*tmp_region_info = *master_region_info;
		...
		DB_WARNING("region_id: %ld, peer_changed: %d, version_changed:%d", region_id, peer_changed, version_changed);
		SchemaManager::get_instance()->process_schema_info(NULL, &request, NULL, NULL);
	} else {
		// 只更新内存
		...
		if (leader_region.status() == pb::IDLE &&
			...
			set_region_mem_info(region_id, leader_region_info.log_index(), leader_region_info.used_size(),
...
void RegionManager::remove_learner_peer(int64_t region_id,
	...
	for (auto& candicate_remove_learner : candicate_remove_learners) {
		...
		if (status != pb::NORMAL) {
			...
			DB_WARNING("abnormal peer: %s because of peers_size:%d status is: %s, region_info: %s",
			           candicate_remove_learner.c_str(), leader_region_info.peers_size(),
			           pb::Status_Name(status).c_str(), leader_region_info.ShortDebugString().c_str());
		}
	}
	...
	if (remove_learner.empty()) {
		for (auto& learner : candicate_remove_learners) {
			...
			DB_WARNING("cadidate remove peer, peer_count: %ld, instance: %s, table_id: %ld", peer_count,
			           learner.c_str(), table_id);
...
void RegionManager::add_learner_peer(int64_t region_id,
                                     std::vector<std::pair<std::string, pb::InitRegion>>& add_learner_requests,
                                     pb::RegionInfo* master_region_info, const std::string& learner_resource_tag) {
	if (!can_modify_learner(region_id)) {
		DB_WARNING("region_id: %ld can't modify learner", region_id);
		return;
	}
	std::string new_instance;
	auto ret = ClusterManager::get_instance()->select_instance_rolling({learner_resource_tag, "", ""},
	                                                                   std::set<std::string>(), new_instance);
	if (ret < 0) {
		DB_FATAL("select store from cluster fail, region_id:%ld, learner_resource_tag: %s, "
		         "peer_size:%d",
		         region_id, learner_resource_tag.c_str(), master_region_info->peers_size());
		return;
	}
	pb::InitRegion request;
	*(request.mutable_region_info()) = *master_region_info;
...
void RegionManager::check_peer_count(
    int64_t region_id, const pb::LeaderHeartBeat& leader_region,
    std::unordered_map<int64_t, int64_t>& table_replica_nums,
    std::unordered_map<int64_t, std::unordered_map<std::string, int>>& table_replica_dists_maps,
    std::vector<std::pair<std::string, pb::RaftControlRequest>>& remove_peer_requests,
    std::set<int64_t>& binlog_table_ids, int32_t table_pk_prefix_dimension, pb::StoreHeartBeatResponse* response) {
	if (leader_region.status() != pb::IDLE) {
		return;
	}
	auto master_region_info = get_region_info(region_id);
	...
	TableManager::get_instance()->get_partition_info(table_id, partition_id, partition_replica_num,
	                                                 partition_resource_tag);
	...
	for (const auto& peer : peers_local_idc) {
		...
		DB_WARNING("table_id: %ld, region_id: %lu, peer: %s, idc: %s not in replicaDist", table_id, region_id,
		           peer.first.c_str(), peer.second.to_string().c_str());
	}
	for (const auto& replica_idc : table_replica_dist) {
		if (table_replica_idc_2_peers[replica_idc.first].size() < replica_idc.second) {
			...
			DB_WARNING(
			    ...
			    table_id, region_id, candidate_idc.c_str(), replica_idc.second,
	...
	if (need_add_peer) {
		...
		auto ret = ClusterManager::get_instance()->select_instance_rolling(idc, peers_in_heart, new_instance);
		if (ret < 0) {
			if (leader_region_info.peers_size() <= replica_num) {
				DB_FATAL("select store from cluster fail, region_id:%ld, "
				         ...
				         region_id, leader_region_info.peers_size(), replica_num, candidate_idc.c_str());
				return;
			}
		} else {
	...
	if (leader_region_info.peers_size() > replica_num) {
		...
		for (auto& candicate_remove_peer : candidate_remove_peers) {
			...
			// 先判断这些peer中是否有peer所在的实例状态不是NORMAL
			if (status == pb::MIGRATE && is_binlog_region &&
			    !binlog_peer_can_delete(candicate_remove_peer, region_id, table_id)) {
			...
			if (status != pb::NORMAL) {
				...
				DB_WARNING("abnormal peer: %s because of peers_size:%d status is: %s, region_info: %s",
				           candicate_remove_peer.c_str(), leader_region_info.peers_size(),
				           pb::Status_Name(status).c_str(), leader_region_info.ShortDebugString().c_str());
				continue;
			}
		}
		...
		if (remove_peer.empty() && !peer_not_in_replica_dist.empty()) {
			...
			if (is_binlog_region && !binlog_peer_can_delete(remove_peer, region_id, table_id)) {
				DB_WARNING("table_id: %ld, region_id: %ld, want remove peer: %s for not in replicaDist. but region "
				           "without enough binlog data, skip",
				           table_id, region_id, remove_peer.c_str());
				return;
			}
			DB_WARNING("table_id: %ld, region_id: %ld, remove peer: %s, because of not in replicaDist.", table_id,
			           region_id, remove_peer.c_str());
		}
		...
		if (remove_peer.empty()) {
			for (const auto& replica_idc : table_replica_dist) {
				if (table_replica_idc_2_peers[replica_idc.first].size() > replica_idc.second) {
					...
					DB_WARNING("table_id: %ld, region_id: %ld, candicate remove idc: %s, peer count: %ld", table_id,
					           region_id, replica_idc.first.c_str(),
					           table_replica_idc_2_peers[replica_idc.first].size());
					break;
				}
			}

			// 如果表开启了pk_prefix balance，判断是否是pk_prefix发起的迁移，有就从pk_prefix维度remove
			// peer数最多的，否则走table的维度 避免pk_prefix维度，和table维度反复peer迁移
			IdcInfo pk_prefix_idc;
			if (table_pk_prefix_dimension > 0 && need_remove_peer_on_pk_prefix(region_id, pk_prefix_idc)) {
				bool get_pk_prefix_success = TableManager::get_instance()->get_pk_prefix_key(
				    table_id, table_pk_prefix_dimension, leader_region_info.start_key(), pk_prefix_key);
				if (get_pk_prefix_success) {
					for (auto& peer : candidate_remove_peers) {
						if (!peers_local_idc[peer].match(pk_prefix_idc)) {
							continue;
						}
						int64_t peer_pk_prefix_region_count =
						    ClusterManager::get_instance()->get_instance_pk_prefix_peer_count(peer, pk_prefix_key);
			...
			if (remove_peer.empty()) {
				...
				for (auto& peer : candidate_remove_peers) {
					...
					DB_WARNING("candidate remove peer, peer_count: %ld, instance: %s, table_id: %ld", peer_count,
					           peer.c_str(), table_id);
					if (peer_count >= max_peer_count) {
						remove_peer = peer;
						max_peer_count = peer_count;
		...
		DB_WARNING("remove peer, peer_count: %ld, instance: %s, table_id: %ld, region_info:%s", max_peer_count,
		           remove_peer.c_str(), table_id, leader_region_info.ShortDebugString().c_str());
		for (auto& peer : leader_region_info.peers()) {
...
void RegionManager::check_whether_illegal_peer(const pb::StoreHeartBeatRequest* request,
	...
	for (auto& peer_info : request->peer_infos()) {
		...
		if (master_region_info == nullptr) {
			...
			// 2.空region merge过程中region
			// 区间已经置空但是还未删除store上的region，此时meta重启此region不会再次读入内存
			if (peer_info.has_start_key() && peer_info.has_end_key() && !peer_info.start_key().empty() &&
				...
				if (peer_info.start_key() == peer_info.end_key()) {
					DB_WARNING("region_id:%ld is none peer, "
					           ...
					           region_id, peer_info.ShortDebugString().c_str(), instance.c_str());
		...
		if (master_region_info->log_index() > peer_info.log_index()) {
			...
			if (!legal_peer) {
				DB_WARNING("region_id:%ld is not legal peer, log_index:%ld,"
				           ...
				           region_id, master_region_info->log_index(), master_region_info->ShortDebugString().c_str(),
				           peer_info.ShortDebugString().c_str(), instance.c_str());
				response->add_delete_region_ids(region_id);
				_region_peer_state_map.erase(region_id);
			...
			if (master_region_info->start_key() == master_region_info->end_key()) {
				if (peer_info.start_key() == peer_info.end_key()) {
					DB_WARNING("region_id:%ld is none peer, log_index:%ld,"
					           ...
					           master_region_info->ShortDebugString().c_str(), peer_info.ShortDebugString().c_str(),
					           instance.c_str());
					response->add_delete_region_ids(region_id);
					_region_peer_state_map.erase(region_id);
					...
					DB_WARNING("region_id:%ld is none peer, master_log_index:%ld, peer_log_index:%ld,"
					           " master_peer_info: %s, peer_info:%s, peer_address:%s should be delete",
					           region_id, master_region_info->log_index(), peer_info.log_index(),
					           master_region_info->ShortDebugString().c_str(), peer_info.ShortDebugString().c_str(),
					           instance.c_str());
					response->add_delete_region_ids(region_id);
					_region_peer_state_map.erase(region_id);
...
int RegionManager::load_region_snapshot(const std::string& key, const std::string& value) {
	...
	if (region_pb.start_key() == region_pb.end_key() && !region_pb.start_key().empty()) {
		// 空region不再读取
	...
	int ret = TableManager::get_instance()->whether_exist_table_partition(table_id, partition_id);
	if (ret < 0) {
		DB_WARNING("region_id: %ld, table_id: %ld, partition_id: %ld not exist, skip", region_id, table_id,
		           partition_id);
		return 0;
	}
	...
	TableManager::get_instance()->add_region_id(region_pb.table_id(), region_pb.partition_id(), region_pb.region_id());
...
void RegionManager::migirate_region_for_store(const std::string& instance) {
	...
	DB_FATAL("instance used size exceed 60%% of capacity, please migirate,"
	         "instance:%s, regions:%s",
	         instance.c_str(), regions_string.c_str());
}
...
void RegionManager::region_healthy_check_function() {
	...
	for (auto& region_id : region_ids) {
		...
		if (region_info->start_key() == region_info->end_key() && !region_info->start_key().empty()) {
			// 长时间没有收到空region的心跳，说明store已经删除，此时meta也可删除
			DB_WARNING("region_id:%ld, table_id: %ld leader:%s maybe erase", region_id, region_info->table_id(),
			           region_info->leader().c_str());
			drop_region_ids.push_back(region_id);
			continue;
		}
		DB_WARNING("region_id:%ld not recevie heartbeat for a long time, table_id: %ld leader:%s", region_id,
		           region_info->table_id(), region_info->leader().c_str());
	...
	// set learner peer
	_region_learner_peer_state_map.traverse_with_key_value([this](int64_t region_id, RegionLearnerState& learner_map) {
...
void RegionManager::erase_region_info(const std::vector<int64_t>& drop_region_ids,
	...
	for (auto drop_region_id : drop_region_ids) {
		...
		TableManager::get_instance()->erase_region(table_id, drop_region_id, region_ptr->start_key(),
		                                           region_ptr->partition_id());
...
bool RegionManager::check_table_in_resource_tags(int64_t table_id, const std::set<std::string>& resource_tags) {
...
void RegionManager::remove_error_peer(const int64_t region_id, const std::set<std::string>& resource_tags,
                                      std::set<std::string> peers, std::vector<pb::PeerStateInfo>& recover_region_way) {
	SmartRegionInfo region_ptr = _region_info_map.get(region_id);
	if (region_ptr == nullptr) {
	...
	for (auto& peer : peers) {
		...
		DB_WARNING("send get_applied_index to %s request:%s, response:%s", peer.c_str(),
		           store_request.ShortDebugString().c_str(), res.ShortDebugString().c_str());
	...
	DB_WARNING("send SetPeer request:%s, response:%s", request.ShortDebugString().c_str(),
	           response.ShortDebugString().c_str());
	if (ret == 0) {
...
void RegionManager::remove_illegal_peer(const int64_t region_id, const std::set<std::string>& resource_tags,
                                        std::set<std::string> peers,
	...
	for (auto& peer : peers) {
		...
		DB_WARNING("send remove region to store:%s request: %s, resposne: %s, ret: %d", peer.c_str(),
		           remove_region_request.ShortDebugString().c_str(), remove_region_response.ShortDebugString().c_str(),
		           ret);
		if (ret == 0) {
			pb::PeerStateInfo peer_status;
			peer_status.set_region_id(region_id);
...
void RegionManager::recovery_single_region_by_set_peer(const int64_t region_id,
                                                       ...
                                                       const pb::RecoverOpt recover_opt, std::set<std::string> peers,
	...
	for (auto& peer : peers) {
		...
		pb::StoreRes res;
		auto ret = store_interact.send_request("get_applied_index", store_request, res);
		DB_WARNING("send get_applied_index request:%s, response:%s", store_request.ShortDebugString().c_str(),
	...
	if (has_alive_peer && recover_opt == pb::DO_SET_PEER) {
		...
		if (ret == 0) {
			...
			DB_FATAL("send SetPeer failed, request:%s, response:%s", request.ShortDebugString().c_str(),
			         response.ShortDebugString().c_str());
		}
	} else {
...
void RegionManager::recovery_single_region_by_init_region(const std::set<int64_t> region_ids,
	...
	for (int64_t region_id : region_ids) {
		...
		if (iter != exist_regions.end()) {
			...
			auto send_set_peer_func = [this, region_id, select_info, max_log_index, region_ptr, &recover_region_way]() {
				pb::RaftControlRequest request;
				request.set_op_type(pb::SetPeer);
				request.set_region_id(region_id);
				...
				pb::RaftControlResponse response;
				int ret = store_interact.send_request_for_leader("region_raft_control", request, response);
				DB_WARNING("send SetPeer request:%s, response:%s max_log_index:%ld", request.ShortDebugString().c_str(),
				           response.ShortDebugString().c_str(), max_log_index);
				if (ret == 0) {
				...
				do {
					...
					*(init_region_request.mutable_region_info()) = region_info;
					...
					DB_WARNING("send init_region request:%s, response:%s",
					           init_region_request.ShortDebugString().c_str(), res.ShortDebugString().c_str());
					if (ret < 0) {
...
void RegionManager::recovery_all_region(const pb::MetaManagerRequest& request, pb::MetaManagerResponse* response) {
	...
	while (iter != region_peers_map.cend()) {
		...
		if (recover_opt == pb::DO_REMOVE_PEER) {
			...
			auto recovery_func = [this, region_id, resource_tags, recover_opt, peers, &not_alive_regions,
			                      &recover_region_way]() {
				recovery_single_region_by_set_peer(region_id, resource_tags, recover_opt, peers, not_alive_regions,
				                                   recover_region_way);
			};
