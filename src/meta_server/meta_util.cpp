...
int check_partition_expr(const pb::Expr& expr, bool lower_or_upper_bound) {
	...
	if (!lower_or_upper_bound) {
		if (compare(expr, partition_min_expr) < 0 || compare(expr, partition_max_expr) >= 0) {
			DB_WARNING("Expr not in valid range, bound: %d, expr: %s, in: %s, max: %s", lower_or_upper_bound,
			           expr.ShortDebugString().c_str(), partition_min_expr.ShortDebugString().c_str(),
	...
	if (lower_or_upper_bound) {
		if (compare(expr, partition_min_expr) <= 0 || compare(expr, partition_max_expr) > 0) {
			DB_WARNING("Expr not in valid range, bound: %d, expr: %s, min: %s, max: %s", lower_or_upper_bound,
			           expr.ShortDebugString().c_str(), partition_min_expr.ShortDebugString().c_str(),
			           partition_max_expr.ShortDebugString().c_str());
			return -1;
		}
...
int create_dynamic_range_partition_info(const std::string& partition_prefix, const pb::PrimitiveType partition_col_type,
...
bool check_partition_overlapped(const std::vector<pb::RangePartitionInfo>& partitions_vec,
...
// 两个区间左端点的较大值「小于」两个区间右端点的较小值，则区间重叠
bool check_partition_overlapped(const ::google::protobuf::RepeatedPtrField<pb::RangePartitionInfo>& partitions_vec,
                                const pb::RangePartitionInfo& partition) {
	const pb::Expr& new_left_value = partition.range().left_value();
	const pb::Expr& new_right_value = partition.range().right_value();
	for (auto& range_partition_info : partitions_vec) {
...
// 获取「小于等于」指定时间的分区
int get_specifed_partitions(const pb::Expr& specified_expr,
...
// 将less_value转换成range
int convert_to_partition_range(const pb::PrimitiveType partition_col_type,
                               pb::RangePartitionInfo& range_partition_info) {
	if (!range_partition_info.has_less_value()) {
		return 0;
	}
...
int set_partition_col_type(const pb::PrimitiveType partition_col_type, pb::RangePartitionInfo& range_partition_info) {
	if (range_partition_info.has_less_value()) {
...
int get_partition_range(const pb::RangePartitionInfo& range_partition_info,
