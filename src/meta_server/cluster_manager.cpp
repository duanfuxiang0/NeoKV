...
bvar::Window<bvar::Adder<int>> g_neometa_min_fallback_count("cluster_manager", "min_fallback_count",
                                                            &min_fallback_count, 3600);
bvar::Window<bvar::Adder<int>> g_neometa_min_count("cluster_manager", "min_count", &min_count, 3600);
bvar::Window<bvar::Adder<int>> g_neometa_rolling_fallback_count("cluster_manager", "rolling_fallback_count",
...
void ClusterManager::drop_logical(const pb::MetaManagerRequest& request, braft::Closure* done) {
	...
	auto ret = MetaRocksdb::get_instance()->write_meta_info(std::vector<std::string> {construct_logical_key()},
	                                                        std::vector<std::string> {value}, drop_logical_keys);
	if (ret < 0) {
...
void ClusterManager::add_physical(const pb::MetaManagerRequest& request, braft::Closure* done) {
	...
	if (ret < 0) {
		DB_WARNING("add logical room: %s to rocksdb fail", request.ShortDebugString().c_str());
		IF_DONE_SET_RESPONSE(done, pb::INTERNAL_ERROR, "write db fail");
		return;
	}
	// 更新内存值
...
void ClusterManager::drop_physical(const pb::MetaManagerRequest& request, braft::Closure* done) {
	...
	for (auto drop_room : logical_physical_room.physical_rooms()) {
		...
		if (_physical_info[drop_room] != logical_room) {
			DB_WARNING("physical room:%s not belong to logical_room:%s", drop_room.c_str(), logical_room.c_str());
			IF_DONE_SET_RESPONSE(done, pb::INPUT_PARAM_ERROR, "physical not exist");
			return;
		}
		// 物理机房下不能有实例
		if (_physical_instance_map.count(drop_room) > 0 && _physical_instance_map[drop_room].size() != 0) {
...
void ClusterManager::add_instance(const pb::MetaManagerRequest& request, braft::Closure* done) {
	...
	// 物理机房不存在
	if (_physical_info.find(physical_room) == _physical_info.end()) {
		DB_WARNING("physical room:%s not exist, instance:%s", physical_room.c_str(), address.c_str());
...
void ClusterManager::drop_instance(const pb::MetaManagerRequest& request, braft::Closure* done) {
	...
	// write date to rocksdb
	auto ret =
	    MetaRocksdb::get_instance()->delete_meta_info(std::vector<std::string> {construct_instance_key(address)});
	if (ret < 0) {
...
void ClusterManager::update_instance(const pb::MetaManagerRequest& request, braft::Closure* done) {
	...
	if (resource_tag_changed) {
		...
		auto call_func = [](std::unordered_map<std::string, InstanceSchedulingInfo>& scheduling_info,
		                    const std::string& address, const IdcInfo& new_idc) -> int {
...
void ClusterManager::add_neo_instance(const pb::BaikalStatus& neo_status) {
	...
	if (address.empty() || resource_tag.empty()) {
		...
		DB_FATAL("neo address or resource tag empty when try to add neo instance. address :%s, resource tag:%s",
		         address.c_str(), resource_tag.c_str());
		return;
	}
...
void ClusterManager::update_neo_instance(const pb::BaikalStatus& neo_status) {
	...
	DB_NOTICE("modify neo resourece tag success, neo address:%s , new resource_tag:%s", address.c_str(),
	          resource_tag.c_str());
}
...
void ClusterManager::move_physical(const pb::MetaManagerRequest& request, braft::Closure* done) {
	...
	if (_physical_info[physical_room] != old_logical_room) {
		DB_WARNING("physical room:%s not belong to old logical room:%s", physical_room.c_str(),
		           old_logical_room.c_str());
		IF_DONE_SET_RESPONSE(done, pb::INPUT_PARAM_ERROR, "physical room not belong to old logical room");
		return;
	}
...
void ClusterManager::set_instance_migrate(const pb::MetaManagerRequest* request, pb::MetaManagerResponse* response,
                                          uint64_t log_id) {
	...
	DB_WARNING("instance:%s region size:%lu, learner size:%lu", instance.c_str(), region_ids.size(),
	           learner_ids.size());
...
void ClusterManager::set_instance_status(const pb::MetaManagerRequest* request, pb::MetaManagerResponse* response,
                                         uint64_t log_id) {
	response->set_op_type(request->op_type());
	response->set_errcode(pb::SUCCESS);
...
void ClusterManager::process_pk_prefix_load_balance(
	...
	for (auto& pk_prefix_region : pk_prefix_region_counts) {
		...
		DB_DEBUG("handle table_id: %s, key: %s, total_peer: %lu, total_instance_count: %lu, "
		         ...
		         pk_prefix_region.first.substr(0, table_id_end).c_str(), pk_prefix_region.first.c_str(),
		         pk_prefix_total_count, total_instance_count, average_peer_count, pk_prefix_region.second,
		...
		if (pk_prefix_region.second > (size_t)(average_peer_count + average_peer_count * 5 / 100)) {
			...
			DB_DEBUG("table_id: %lu, pk_prefix: %s, need add %lu, average: %lu, table dimension need add: %lu",
			         table_id, pk_prefix_region.first.c_str(), pk_prefix_add_peer_counts[pk_prefix_region.first],
			         pk_prefix_average_counts[pk_prefix_region.first], table_add_peer_counts[table_id]);
		}
	}
...
void ClusterManager::process_peer_heartbeat_for_store(const pb::StoreHeartBeatRequest* request,
	...
	for (auto& peer_info : request->peer_infos()) {
		...
		std::string key;
		if (!TableManager::get_instance()->get_pk_prefix_key(
		        peer_info.table_id(), table_pk_prefix_dimension[peer_info.table_id()], peer_info.start_key(), key)) {
			DB_WARNING("decode pk_prefix_key fail, table_id: %lu, region_id: %lu", peer_info.table_id(),
			           peer_info.region_id());
			continue;
		}
	...
	if (!_meta_state_machine->whether_can_decide()) {
		DB_WARNING("meta state machine can not make decision, resource_tag: %s, instance: %s", resource_tag.c_str(),
		           instance.c_str());
		return;
	}
	if (!_meta_state_machine->get_load_balance(resource_tag)) {
		DB_WARNING("meta state machine close peer load balance, resource_tag: %s, instance: %s", resource_tag.c_str(),
		           instance.c_str());
		return;
	}
	...
	DB_WARNING("peer load balance, instance_info: %s, idc: %s", instance.c_str(), instance_idc.to_string().c_str());
	...
	for (auto& table_region : table_regions) {
		...
		DB_DEBUG("process tableid %ld region size %zu average cout %zu, idc: %s, total_count: %ld", table_id,
		         table_region.second.size(), (size_t)(average_peer_count + average_peer_count * 5 / 100),
	...
	if (!pk_prefix_region_counts.empty() && TableManager::get_instance()->can_do_pk_prefix_balance()) {
		...
		process_pk_prefix_load_balance(pk_prefix_region_counts, table_balance_idc, idc_instance_count, add_peer_counts,
		                               pk_prefix_add_peer_counts, pk_prefix_average_counts);
	}
	if (!pk_prefix_add_peer_counts.empty()) {
		RegionManager::get_instance()->pk_prefix_load_balance(pk_prefix_add_peer_counts, pk_prefix_regions, instance,
		                                                      table_balance_idc, pk_prefix_average_counts,
	...
	for (auto& add_peer_count : add_peer_counts) {
		DB_WARNING("instance: %s should add peer count for peer_load_balance, "
		           ...
		           instance.c_str(), add_peer_count.first, add_peer_count.second,
		           table_balance_idc[add_peer_count.first].to_string().c_str());
	}
	if (add_peer_counts.size() > 0) {
		RegionManager::get_instance()->peer_load_balance(add_peer_counts, table_regions, instance, table_balance_idc,
		                                                 table_average_counts, table_pk_prefix_dimension,
	...
	for (auto& add_learner_count : add_learner_counts) {
		DB_WARNING("instance: %s should add learner count for learner_load_balance, "
		           ...
		           instance.c_str(), add_learner_count.first, add_learner_count.second, resource_tag.c_str());
	}
	if (add_learner_counts.size() > 0) {
		RegionManager::get_instance()->learner_load_balance(add_learner_counts, table_regions, instance, resource_tag,
		                                                    table_average_counts);
	} else {
...
void ClusterManager::store_healthy_check_function() {
	...
	std::unordered_map<std::string, std::vector<Instance>> migrate_stores;
	{
		...
		for (auto& instance_pair : _instance_info) {
			...
			if ((butil::gettimeofday_us() - last_timestamp) >
				...
				DB_WARNING("instance:%s is dead DEAD, resource_tag: %s", instance_pair.first.c_str(),
				           resource_tag.c_str());
				std::vector<int64_t> region_ids;
			...
			if ((butil::gettimeofday_us() - last_timestamp) >
				...
				DB_WARNING("instance:%s is faulty FAULTY, resource_tag: %s", instance_pair.first.c_str(),
				           resource_tag.c_str());
	...
	for (auto& dead_store_pair : dead_stores) {
		...
		if ((dead_store_num[resource_tag] + faulty_store_num[resource_tag]) * 100 / total_store_num[resource_tag] >=
		        FLAGS_error_judge_percent &&
	...
	for (auto& store_pair : dead_stores) {
		for (auto& store : store_pair.second) {
			DB_WARNING("store:%s is dead, resource_tag: %s", store.address.c_str(), store_pair.first.c_str());
			RegionManager::get_instance()->delete_all_region_for_store(store.address, store.instance_status);
		}
	}
	for (auto& store_pair : migrate_stores) {
		...
		std::sort(store_pair.second.begin(), store_pair.second.end(), [](const Instance& l, const Instance& r) {
			return l.instance_status.state_duration.get_time() > r.instance_status.state_duration.get_time();
		});
		for (auto& store : store_pair.second) {
			if (_meta_state_machine->get_migrate(store_pair.first) &&
			    ...
			    RegionManager::get_instance()->check_binlog_regions_can_migrate(
			        store.address) // 检查store上所有的binlog有其他数据超过7天的peer
			    && store.instance_status.state_duration.get_time() > delay * 1000 * 1000 && concurrency-- > 0) {
				DB_WARNING("store:%s is migrating, resource_tag: %s", store.address.c_str(), store_pair.first.c_str());
				RegionManager::get_instance()->add_peer_for_store(store.address, store.instance_status);
			} else {
				DB_WARNING("store:%s wait for migrate, resource_tag: %s, fast_import:%d, concurrency:%ld",
				           ...
				           TableManager::get_instance()->is_cluster_in_fast_importer(store_pair.first), concurrency);
			}
		}
	}
	// 若实例满，则做实例迁移
...
void ClusterManager::auto_network_segments_division(std::string resource_tag) {
	...
	DB_WARNING("finish auto network segment division for resource tag: %s, prefix: %d", resource_tag.c_str(), prefix);
}

int ClusterManager::select_instance_min_on_pk_prefix(const IdcInfo& idc, const std::set<std::string>& exclude_stores,
                                                     const int64_t& table_id, const std::string& pk_prefix_key,
                                                     std::string& selected_instance,
                                                     const int64_t& pk_prefix_average_count,
                                                     const int64_t& table_average_count, bool need_both_below_average) {
	auto pick_candidate_instances =
	    [this, idc, exclude_stores, table_id, pk_prefix_key, pk_prefix_average_count,
	     table_average_count](std::vector<std::string>& instances, std::vector<std::string>& candidate_instances,
	...
	BAIDU_SCOPED_LOCK(_instance_mutex);
	if (_resource_tag_instance_map.count(resource_tag) == 0 || _resource_tag_instance_map[resource_tag].empty()) {
	...
	if (_meta_state_machine->get_network_segment_balance(resource_tag)) {
		...
		for (auto& network_segment : instances_by_network) {
			...
			pick_candidate_instances(network_segment.second, candidate_instances,
		...
		if (candidate_instances.empty()) {
			...
			for (auto& network_segment : exclude_network_segment) {
				pick_candidate_instances(instances_by_network[network_segment], candidate_instances,
				                         candidate_instances_pk_prefix_dimension);
			}
		}
	} else {
	...
	DB_WARNING("select instance min on pk_prefix dimension, table_id: %ld, idc: %s, "
	           ...
	           "selected_instance: %s",
	           table_id, idc.to_string().c_str(), pk_prefix_average_count, table_average_count,
	           candidate_instances.size(), candidate_instances_pk_prefix_dimension.size(), selected_instance.c_str());
	return 0;
}
...
int ClusterManager::select_instance_min(const IdcInfo& idc, const std::set<std::string>& exclude_stores,
                                        const int64_t& table_id, std::string& selected_instance,
                                        const int64_t& average_count) {
	auto pick_candidate_instances = [this, idc, exclude_stores, table_id,
	                                 average_count](std::vector<std::string>& instances,
	                                                std::vector<std::string>& candidate_instances,
	...
	if (_meta_state_machine->get_network_segment_balance(resource_tag)) {
		...
		for (auto& network_segment : instances_by_network) {
			// not overlap
			if (exclude_network_segment.find(network_segment.first) == exclude_network_segment.end()) {
				bool ret = pick_candidate_instances(network_segment.second, candidate_instances, selected_instance,
				                                    max_region_count);
				if (ret) {
					break;
				}
	...
	DB_WARNING("select instance min, table_id: %ld, idc: %s,"
	           ...
	           table_id, idc.to_string().c_str(), average_count, candidate_instances.size(), selected_instance.c_str());
	return 0;
}
...
int ClusterManager::select_instance_rolling(const IdcInfo& idc, const std::set<std::string>& exclude_stores,
                                            std::string& selected_instance) {
	selected_instance.clear();
	const std::string& resource_tag = idc.resource_tag;
	BAIDU_SCOPED_LOCK(_instance_mutex);
	...
	DB_WARNING("select instance rolling, idc: %s, selected_instance: %s", idc.to_string().c_str(),
...
int ClusterManager::load_snapshot() {
	...
	{
		BAIDU_SCOPED_LOCK(_physical_mutex);
		_physical_info[FLAGS_default_physical_room] = FLAGS_default_logical_room;
		_logical_physical_map[FLAGS_default_logical_room] = std::set<std::string> {FLAGS_default_physical_room};
	}
	{
		BAIDU_SCOPED_LOCK(_instance_mutex);
	...
	std::unique_ptr<rocksdb::Iterator> iter(db->new_iterator(read_options, db->get_meta_info_handle()));
...
bool ClusterManager::is_legal_for_select_instance(const IdcInfo& idc, const std::string& candicate_instance,
                                                  const std::set<std::string>& exclude_stores) {
	if (_instance_info.find(candicate_instance) == _instance_info.end()) {
		return false;
	}
	if (!idc.logical_room.empty() && _instance_info[candicate_instance].logical_room != idc.logical_room) {
		return false;
	}
	if (!idc.physical_room.empty() && _instance_info[candicate_instance].physical_room != idc.physical_room) {
		return false;
	}
	...
	if ((_instance_info[candicate_instance].used_size * 100 / _instance_info[candicate_instance].capacity) >
		...
		DB_DEBUG("instance:%s left size is not enough, used_size:%ld, capactity:%ld", candicate_instance.c_str(),
		         _instance_info[candicate_instance].used_size, _instance_info[candicate_instance].capacity);
		return false;
	}
	return true;
}
int ClusterManager::load_instance_snapshot(const std::string& instance_prefix, const std::string& key,
...
int ClusterManager::load_instance_param_snapshot(const std::string& instance_param_prefix, const std::string& key,
                                                 const std::string& value) {
...
int ClusterManager::load_physical_snapshot(const std::string& physical_prefix, const std::string& key,
                                           const std::string& value) {
...
int ClusterManager::load_logical_snapshot(const std::string& logical_prefix, const std::string& key,
                                          const std::string& value) {
...
int ClusterManager::update_instance_info(const pb::InstanceInfo& instance_info) {
	...
	if (_instance_info.find(instance) == _instance_info.end()) {
		...
		if (same_with_container_id_and_address(instance_info.container_id(), instance_info.address())) {
			return -1;
		} else {
			return 0;
		}
	...
	if (status == pb::NORMAL) {
		if (FLAGS_store_rocks_hang_check) {
			...
			if (store_rocks_check_cost >= FLAGS_store_rocks_hang_check_timeout_s * 1000 * 1000LL) {
				...
				DB_WARNING("instance:%s status SLOW, resource_tag: %s,  store_rocks_check_cost: %ld", instance.c_str(),
				           is.resource_tag.c_str(), store_rocks_check_cost);
				return 0;
			}
		}
		...
		if (cnt > 5 && is.raft_total_latency > 100 * all_raft_total_latency / cnt &&
			...
			DB_WARNING("instance:%s status SLOW, resource_tag: %s, raft_total_latency:%ld, dml_latency:%ld, "
			           ...
			           instance.c_str(), is.resource_tag.c_str(), is.raft_total_latency, is.dml_latency, cnt,
			           all_raft_total_latency / cnt, all_dml_latency / cnt);
			status = pb::SLOW;
			_slow_instances.insert(instance);
		}
		...
		if (!FLAGS_need_check_slow) {
			...
			status = pb::NORMAL;
			DB_WARNING("instance:%s status NORMAL, resource_tag: %s, store_rocks_check_cost: %ld", instance.c_str(),
			           is.resource_tag.c_str(), store_rocks_check_cost);
			return 0;
		}
		...
		DB_WARNING("instance:%s status return NORMAL, resource_tag: %s", instance.c_str(), is.resource_tag.c_str());
		status = pb::NORMAL;
	}
	return 0;
}
