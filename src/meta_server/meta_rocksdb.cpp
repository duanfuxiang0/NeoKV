...
int MetaRocksdb::put_meta_info(const std::string& key, const std::string& value) {
	...
	if (!status.ok()) {
		DB_WARNING("put rocksdb fail, err_msg: %s, key: %s, value: %s", status.ToString().c_str(), key.c_str(),
		           value.c_str());
		return -1;
	}
	return 0;
...
int MetaRocksdb::put_meta_info(const std::vector<std::string>& keys, const std::vector<std::string>& values) {
	...
	if (!status.ok()) {
		DB_WARNING("put batch to rocksdb fail, err_msg: %s", status.ToString().c_str());
		return -1;
	}
	return 0;
}
...
int MetaRocksdb::write_meta_info(const std::vector<std::string>& put_keys, const std::vector<std::string>& put_values,
