...
DEFINE_int32(db_request_timeout, 30000, "db as server request timeout, default:30000ms");
DEFINE_int32(db_connect_timeout, 5000, "db as server connect timeout, default:5000ms");
...
DEFINE_int32(remote_compaction_request_timeout, 900000, "remote compaction request timeout, default:900000ms");
DEFINE_int32(remote_compaction_request_file_timeout, 10000, "remote compaction request timeout, default:10000");
DEFINE_int32(remote_compaction_connect_timeout, 5000, "remote compaction connect timeout, default:5000ms");
DEFINE_int32(store_port, 8110, "Server port");
DEFINE_int32(redis_port, 16379, "Redis server port");
DEFINE_bool(redis_cluster_enabled, true, "Enable redis cluster protocol entry");
DEFINE_string(redis_advertise_ip, "", "Redis advertise IP for MOVED/CLUSTER responses (empty = auto detect)");
DEFINE_int32(redis_advertise_port, 0, "Redis advertise port for MOVED/CLUSTER responses (0 = use redis_port)");
// Unified ReadIndex control for both SQL and Redis follower/learner reads
DEFINE_bool(use_read_index, false, "Enable ReadIndex for follower/learner reads (strong consistency)");
DEFINE_int32(redis_ttl_cleanup_interval_s, 60, "Interval in seconds for TTL cleanup (0 = disabled)");
...
int64_t timestamp_diff(timeval _start, timeval _end) {
	return (_end.tv_sec - _start.tv_sec) * 1000000 + (_end.tv_usec - _start.tv_usec); // macro second
}
...
int primitive_type_bytes_len(pb::PrimitiveType type) {
	static std::unordered_map<int32_t, int32_t> _mysql_pb_type_bytes_count = {
	    {pb::INT8, 1},  {pb::INT16, 2},  {pb::INT32, 3},  {pb::INT64, 4},
	    {pb::UINT8, 1}, {pb::UINT16, 2}, {pb::UINT32, 3}, {pb::UINT64, 4}};
...
int get_instance_from_bns(int* ret, const std::string& bns_name, std::vector<std::string>& instances, bool need_alive,
                          bool white_list) {
#ifdef BAIDU_INTERNAL
	instances.clear();
	BnsInput input;
	...
	*ret = webfoot::get_instance_by_service(input, &output);
	...
	if (*ret == webfoot::WEBFOOT_RET_SUCCESS || *ret == webfoot::WEBFOOT_SERVICE_BEYOND_THRSHOLD) {
		for (int i = 0; i < output.instance_size(); ++i) {
			if (output.instance(i).status() == 0 || !need_alive) {
				instances.push_back(output.instance(i).host_ip() + ":" + std::to_string(output.instance(i).port()));
			}
		}
		return 0;
	}
	DB_WARNING("get instance from service fail, bns_name:%s, ret:%d", bns_name.c_str(), *ret);
	return -1;
#else
	return -1;
#endif
...
bool same_with_container_id_and_address(const std::string& container_id, const std::string& address) {
	...
	if (instances[0] == address) {
		...
		DB_WARNING("diff with container_id:%s(%s) and address:%s", container_id.c_str(), instances[0].c_str(),
		           address.c_str());
		return false;
	}
#else
...
#endif
int get_multi_port_from_bns(int* ret, const std::string& bns_name, std::vector<std::string>& instances,
                            bool need_alive) {
#ifdef BAIDU_INTERNAL
	instances.clear();
	BnsInput input;
...
void parse_sample_sql(const std::string& sample_sql, std::string& database, std::string& table, std::string& sql) {
	...
	DB_WARNING("sample_sql: %s, database: %s, table: %s, sql: %s", sample_sql.c_str(), database.c_str(), table.c_str(),
	           sql.c_str());
}

int convert_charset(const pb::Charset& from_charset, const std::string& from_str, const pb::Charset& to_charset,
                    std::string& to_str) {
	to_str = from_str;
