...
namespace neokv {
int StreamReceiver::on_received_messages(brpc::StreamId id, butil::IOBuf* const messages[], size_t size) {
	size_t current_index = 0;
	switch (_state) {
	case ReceiverState::RS_LOG_INDEX: {
		DB_NOTICE("get rs_log_index id[%lu]", id);
		multi_iobuf_action(
		    id, messages, size, &current_index,
	...
	case ReceiverState::RS_FILE_NUM: {
		...
		multi_iobuf_action(
		    ...
		    [this](butil::IOBuf* const message, size_t size) {
			    return message->cutn(reinterpret_cast<char*>(&_file_num) + (sizeof(int8_t) - _to_process_size), size);
		    },
		    &_to_process_size);
	...
	case ReceiverState::RS_META_FILE_SIZE: {
		...
		multi_iobuf_action(
		    ...
		    [this](butil::IOBuf* const message, size_t size) -> size_t {
			    return message->cutn(reinterpret_cast<char*>(&_meta_file_size) + (sizeof(int64_t) - _to_process_size),
			                         size);
		    },
		    &_to_process_size);
	...
	case ReceiverState::RS_META_FILE: {
		...
		if (_to_process_size == 0) {
			...
			if (_file_num == 1) {
				...
				_status = (!_meta_file_streaming.bad() && !_data_file_streaming.bad()) ? pb::StreamState::SS_SUCCESS
	...
	case ReceiverState::RS_DATA_FILE_SIZE: {
		...
		multi_iobuf_action(
		    id, messages, size, &current_index,
		    [this](butil::IOBuf* const message, size_t size) -> size_t {
			    return message->cutn(reinterpret_cast<char*>(&_data_file_size) + (sizeof(int64_t) - _to_process_size),
			                         size);
		    },
		    &_to_process_size);
	...
	case ReceiverState::RS_DATA_FILE: {
		...
		if (_to_process_size == 0) {
			DB_NOTICE("id[%lu] get data_size[%ld] all_size[%ld]", id, _data_file_size,
			          _data_file_size + _meta_file_size + 25);
			_meta_file_streaming.flush();
			_data_file_streaming.flush();
			_status = (!_meta_file_streaming.bad() && !_data_file_streaming.bad()) ? pb::StreamState::SS_SUCCESS
			                                                                       : pb::StreamState::SS_FAIL;
