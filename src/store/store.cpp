...
DEFINE_int64(flush_region_interval_us, 10 * 60 * 1000 * 1000LL, "flush region interval, default(10 min)");
DEFINE_int64(transaction_clear_interval_ms, 5000LL, "transaction clear interval, default(5s)");
DEFINE_int64(binlog_timeout_check_ms, 10 * 1000LL, "binlog timeout check interval, default(10s)");
DEFINE_int64(binlog_fake_ms, 1 * 1000LL, "fake binlog interval, default(1s)");
DEFINE_int64(oldest_binlog_ts_interval_s, 3600LL, "oldest_binlog_ts_interval_s, default(1h)");
DECLARE_int64(flush_memtable_interval_us);
DEFINE_int32(max_split_concurrency, 2, "max split region concurrency, default:2");
DEFINE_int64(none_region_merge_interval_us, 5 * 60 * 1000 * 1000LL, "none region merge interval, default(5 min)");
DEFINE_int64(region_delay_remove_timeout_s, 3600 * 24LL, "region_delay_remove_time_s, default(1d)");
DEFINE_bool(use_approximate_size, true, "use_approximate_size");
DEFINE_bool(use_approximate_size_to_split, false, "if approximate_size > 512M, then split");
...
int Store::init_before_listen(std::vector<std::int64_t>& init_region_ids) {
	...
	if (0 != create_dir_if_not_exist(FLAGS_db_path + "_tmp") ||
	    0 != create_dir_if_not_exist(FLAGS_db_path + "_column")) {
	...
	for (auto& region_info : region_infos) {
		...
		// construct region
		braft::GroupId groupId(std::string("region_") + boost::lexical_cast<std::string>(region_id));
		butil::EndPoint addr;
		...
		SmartRegion region(new (std::nothrow)
		                       Region(_rocksdb, _address, groupId, peerId, region_info, region_id, is_learner));
		if (region == NULL) {
		...
		if (region_info.version() == 0) {
			...
			std::function<void()> check_region_legal_fun = [this, region_id]() {
				check_region_legal_complete(region_id);
			};
			bth.run(check_region_legal_fun);
			DB_WARNING("init region verison is 0, should check region legal. region_id: %ld", region_id);
		}
	}
...
void Store::init_region(google::protobuf::RpcController* controller, const pb::InitRegion* request,
	...
	if (orgin_region != nullptr) {
		// 自动化处理，直接删除这个region
		DB_FATAL("region id has existed when add region, region_id: %ld, log_id:%lu, remote_side:%s", region_id, log_id,
		         remote_side);
	...
	braft::PeerId peerId(addr, 0);
	SmartRegion region(new (std::nothrow) Region(_rocksdb, _address, groupId, peerId, request->region_info(), region_id,
	                                             request->region_info().is_learner()));
	if (region == NULL) {
	...
	DB_WARNING("new region_info:%s. logid:%lu remote_side: %s", request->ShortDebugString().c_str(), log_id,
	           remote_side);
	...
	if (ret < 0) {
		...
		DB_FATAL("region init fail when add region, region_id: %ld, log_id:%lu", region_id, log_id);
		response->set_errcode(pb::INTERNAL_ERROR);
	...
	if (request->region_info().version() == 0) {
		...
		bth.run(check_region_legal_fun);
		DB_WARNING("init region verison is 0, should check region legal. region_id: %ld, log_id: %lu", region_id,
		           log_id);
	}
	auto_decrease.release();
	DB_WARNING("init region sucess, region_id: %ld, log_id:%lu, time_cost:%ld remote_side: %s", region_id, log_id,
	           time_cost.get_time(), remote_side);
}

void Store::region_raft_control(google::protobuf::RpcController* controller, const pb::RaftControlRequest* request,
	...
	if (region == NULL) {
		...
		DB_FATAL("region id:%lu not exist in store, logid:%lu", request->region_id(), log_id);
		return;
	}
	...
	region->raft_control(controller, request, response, done_guard.release());
	if (request->op_type() == pb::TransLeader && response->errcode() == pb::SUCCESS) {
		region->transfer_leader_set_is_leader();
	}
}

void Store::health_check(google::protobuf::RpcController* controller, const pb::HealthCheck* request,
                         pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	response->set_errcode(pb::SUCCESS);
	/*
	   ...
	   + RocksdbVars::get_instance()->rocksdb_scan_time_cost_qps.get_value(bvar::FLAGS_bvar_dump_interval) /
	   FLAGS_get_token_weight; int64_t fetch_token_qps =
	   RocksdbVars::get_instance()->qos_fetch_tokens_qps.get_value(bvar::FLAGS_bvar_dump_interval); bool
	   match_reject_condition = StoreQos::get_instance()->match_reject_condition(); static TimeCost last_print_time;
	    // 三个条件满足其一就认为是BUSY
...
void Store::query(google::protobuf::RpcController* controller, const pb::StoreReq* request, pb::StoreRes* response,
	...
	// DB_WARNING("region_id: %ld before get_region, logid:%lu, remote_side: %s", request->region_id(), log_id,
	// remote_side);
	SmartRegion region = get_region(request->region_id());
	if (region == nullptr || region->removed()) {
		response->set_errcode(pb::REGION_NOT_EXIST);
		response->set_errmsg("region_id not exist in store");
		DB_WARNING("region_id: %ld not exist in store, logid:%lu, remote_side: %s", request->region_id(), log_id,
		           remote_side);
		return;
	}
	region->query(controller, request, response, done_guard.release());
}

void Store::query_batch(google::protobuf::RpcController* controller, const pb::BatchRegionStoreReq* request,
	...
	for (int i = 0; i < n; i++) {
		auto batch_query_call = [this, &controller, &batch_response, &non_const_request, i, n, log_id, remote_side]() {
			...
			region->query(controller, single_region_req, &single_response, nullptr);
			single_response.set_orig_region_id(single_region_req->region_id());
			batch_response->mutable_store_res(i)->Swap(&single_response);
			single_region_req->release_plan();
			return;
...
void Store::remove_region(google::protobuf::RpcController* controller, const pb::RemoveRegion* request,
                          pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	brpc::Controller* cntl = static_cast<brpc::Controller*>(controller);

	DB_WARNING("receive remove region request, remote_side:%s, request:%s",
	           butil::endpoint2str(cntl->remote_side()).c_str(), request->ShortDebugString().c_str());
	if (_shutdown) {
		DB_WARNING("store has entered shutdown");
		response->set_errcode(pb::INPUT_PARAM_ERROR);
		response->set_errmsg("store has shutdown");
	...
	DB_WARNING("call remove region_id: %ld, need_delay_drop:%d", request->region_id(), request->need_delay_drop());
	drop_region_from_store(request->region_id(), request->need_delay_drop());
	response->set_errcode(pb::SUCCESS);
}

void Store::restore_region(google::protobuf::RpcController* controller, const neokv::pb::RegionIds* request,
                           pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	response->set_errcode(pb::SUCCESS);
...
void Store::add_peer(google::protobuf::RpcController* controller, const pb::AddPeer* request, pb::StoreRes* response,
                     google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	response->set_errcode(pb::SUCCESS);
	response->set_errmsg("success");
...
void Store::get_applied_index(google::protobuf::RpcController* controller, const neokv::pb::GetAppliedIndex* request,
                              pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	response->set_errcode(pb::SUCCESS);
	response->set_errmsg("success");
...
void Store::compact_region(google::protobuf::RpcController* controller, const neokv::pb::RegionIds* request,
                           pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	response->set_errcode(pb::SUCCESS);
	response->set_errmsg("success");
	...
	if (request->region_ids_size() == 0) {
		...
		bth.run([this, cf]() {
			...
			if (!res.ok()) {
				DB_WARNING("compact_range error: code=%d, msg=%s", res.code(), res.ToString().c_str());
			}
...
void Store::manual_split_region(google::protobuf::RpcController* controller, const neokv::pb::RegionIds* request,
                                pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	response->set_errcode(pb::SUCCESS);
	response->set_errmsg("success");
...
void Store::snapshot_region(google::protobuf::RpcController* controller, const neokv::pb::RegionIds* request,
                            pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	response->set_errcode(pb::SUCCESS);
	response->set_errmsg("success");
	...
	if (request->region_ids_size() == 0) {
		traverse_region_map(
		    [&region_ids](const SmartRegion& region) { region_ids.push_back(region->get_region_id()); });
	} else {
		for (auto region_id : request->region_ids()) {
...
void Store::query_region(google::protobuf::RpcController* controller, const neokv::pb::RegionIds* request,
                         pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	response->set_errcode(pb::SUCCESS);
	response->set_errmsg("success");
	...
	if (request->region_ids_size() == 0) {
		traverse_region_map([&id_leader_map](const SmartRegion& region) {
			id_leader_map[region->get_region_id()] = butil::endpoint2str(region->get_leader()).c_str();
		});
...
void Store::query_file_system(google::protobuf::RpcController* controller, const pb::CompactionFileRequest* request,
                              pb::CompactionFileResponse* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	int ret = 0;
	...
	if (ret != 0) {
		DB_FATAL("remote_compaction_id: %s fail to do file operation, op: %s", request->remote_compaction_id().c_str(),
...
int Store::read_file(const pb::CompactionFileRequest* request, pb::CompactionFileResponse* response) {
	TimeCost cost;
	...
	if (bytes_read < 0) {
		DB_FATAL("Failed to read file: %s, remote_compaction_id: %s", file_path_str.c_str(),
...
int Store::write_file(const pb::CompactionFileRequest* request, pb::CompactionFileResponse* response) {
	TimeCost cost;
	static bvar::LatencyRecorder remote_compaction_write_rate("remote_compaction_write_rate");
	std::vector<std::string> split_vec;
	boost::split(split_vec, request->file_name(), boost::is_any_of("/"));
	const std::string& file_path_str =
	    FLAGS_db_path + "/" + FLAGS_secondary_db_path + "/" + request->remote_compaction_id() + "/" + split_vec.back();
	...
	if (!butil::PathExists(directory_path)) {
		if (!butil::CreateDirectory(directory_path)) {
			DB_FATAL("Failed to create directory: %s, remote_compaction_id: %s", directory_path.value().c_str(),
			         request->remote_compaction_id().c_str());
			return -1;
		}
	}
	...
	if (request->offset() == 0) {
		...
		if (!success) {
			DB_FATAL("Failed to delete file: %s, remote_compaction_id: %s", file_path_str.c_str(),
			         request->remote_compaction_id().c_str());
			return -1;
		}
	}
	...
	if (!file.IsValid()) {
		DB_FATAL("File does not exist: %s, remote_compaction_id: %s", file_path_str.c_str(),
		         request->remote_compaction_id().c_str());
		return -1;
	}
	...
	if (bytes_written < 0 || bytes_written != size) {
		DB_FATAL("Failed to write file: %s, remote_compaction_id: %s", file_path_str.c_str(),
		         request->remote_compaction_id().c_str());
		return -1;
	}
...
int Store::create_dir(const pb::CompactionFileRequest* request, pb::CompactionFileResponse* response) {
	const std::string& dir = FLAGS_db_path + "/" + FLAGS_secondary_db_path + "/" + request->remote_compaction_id();
...
int Store::read_dir(const pb::CompactionFileRequest* request, pb::CompactionFileResponse* response) {
	butil::FilePath file_path(FLAGS_db_path);
	// 检查目录是否存在
	if (!butil::DirectoryExists(file_path)) {
		DB_WARNING("File does not exist: %s, remote_compaction_id: %s", FLAGS_db_path.c_str(),
		           request->remote_compaction_id().c_str());
	...
	// DB_WARNING("remote_compaction_id: %s, response debug: %s", request->remote_compaction_id().c_str(),
	// response->ShortDebugString().c_str());
	return 0;
}

int Store::path_exists(const pb::CompactionFileRequest* request, pb::CompactionFileResponse* response) {
	std::vector<std::string> split_vec;
	boost::split(split_vec, request->file_name(), boost::is_any_of("/"));
	...
	if (!butil::PathExists(file_path)) {
		DB_WARNING("File does not exist: %s, remote_compaction_id: %s", file_path_str.c_str(),
		           request->remote_compaction_id().c_str());
		response->set_errcode(pb::COMPACTION_FILE_NOT_EXIST);
		return 0;
	}
...
int Store::get_file_info_list(const pb::CompactionFileRequest* request, pb::CompactionFileResponse* response) {
	butil::FilePath file_path(FLAGS_db_path);
	// 检查目录是否存在
	if (!butil::DirectoryExists(file_path)) {
		DB_WARNING("File does not exist: %s, remote_compaction_id: %s", FLAGS_db_path.c_str(),
...
int Store::delete_path(const pb::CompactionFileRequest* request, pb::CompactionFileResponse* response) {
	const std::string& dir_str = FLAGS_db_path + "/" + FLAGS_secondary_db_path + "/" + request->remote_compaction_id();
...
void Store::query_illegal_region(google::protobuf::RpcController* controller, const neokv::pb::RegionIds* request,
                                 pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	response->set_errcode(pb::SUCCESS);
	response->set_errmsg("success");
...
void Store::heart_beat_thread() {
	...
	while (!_shutdown) {
		send_heart_beat();
		traverse_copy_region_map([](const SmartRegion& region) { region->cancel_all_blacklist_sign(); });
	...
	while (!_shutdown) {
		...
		traverse_copy_region_map([this](const SmartRegion& region) {
			...
			int64_t random_remove_data_timeout = (FLAGS_region_delay_remove_timeout_s +
			                                      (int64_t)(butil::fast_rand() % FLAGS_region_delay_remove_timeout_s)) *
			                                     1000 * 1000LL;
			if (region->removed() && region->removed_time_cost() > random_remove_data_timeout) {
...
int64_t Store::get_tso() {
	...
	ON_SCOPE_EXIT(([this]() { _get_tso_cond.decrease_signal(); }));
	...
	if (_tso_server_interact.send_request("tso_service", request, response) == 0) {
		...
		if (response.errcode() != pb::SUCCESS) {
			DB_FATAL("store get tso fail request:%s, response:%s", request.ShortDebugString().c_str(),
			         response.ShortDebugString().c_str());
			return -1;
		}
		DB_WARNING("store get tso request:%s, response:%s", request.ShortDebugString().c_str(),
		           response.ShortDebugString().c_str());
	} else {
		DB_WARNING("store get tso request:%s, response:%s", request.ShortDebugString().c_str(),
		           response.ShortDebugString().c_str());
...
void Store::process_merge_request(int64_t table_id, int64_t region_id) {
	...
	if (_meta_server_interact.send_request("meta_manager", request, response) == 0) {
		// 处理响应
		if (response.errcode() != pb::SUCCESS) {
			DB_FATAL("store process merge fail request:%s, response:%s", request.ShortDebugString().c_str(),
			         response.ShortDebugString().c_str());
			return;
		}
		SmartRegion region = get_region(region_id);
		...
		DB_WARNING("store process merge request:%s, response:%s", request.ShortDebugString().c_str(),
		           response.ShortDebugString().c_str());
...
void Store::process_split_request(int64_t table_id, int64_t region_id, bool tail_split, const std::string& split_key,
                                  int64_t key_term) {
	++_split_num;
	// 构造请求
	pb::MetaManagerRequest request;
	...
	if (_meta_server_interact.send_request("meta_manager", request, response) == 0) {
		...
		DB_WARNING("store process split request:%s, response:%s", request.ShortDebugString().c_str(),
		           response.ShortDebugString().c_str());
...
void Store::monitor_memory() {
	...
	for (auto kv : usage_by_type) {
		if (_rocksdb->get_cache() != nullptr) {
			DB_WARNING("momery type: %d, size: %lu, %lu", kv.first, kv.second, _rocksdb->get_cache()->GetPinnedUsage());
		}
	}
}
...
void Store::check_region_peer_delay() {
	while (!_shutdown) {
		...
		std::vector<int64_t> region_ids;
		traverse_region_map(
		    [&region_ids](const SmartRegion& region) { region_ids.emplace_back(region->get_region_id()); });
...
void Store::whether_split_thread() {
	...
	while (!_shutdown) {
		...
		for (size_t i = 0; i < region_ids.size(); ++i) {
			...
			int64_t split_key_term = 0;
			// 如果是尾部分
			if (ptr_region->is_leader() && ptr_region->get_status() == pb::IDLE &&
			    _split_num.load() < FLAGS_max_split_concurrency) {
				if (ptr_region->is_tail() && ptr_region->get_num_table_lines() >= region_capacity) {
					...
					DB_WARNING("start split by approx size:%ld region_id: %ld num_table_lines:%ld", region_sizes[i],
					           region_ids[i], ptr_region->get_num_table_lines());
			...
			if (ptr_region->is_leader() &&
			    (ptr_region->get_num_table_lines() == 0 || ptr_region->get_used_size() == 0) &&
	...
	while (!_shutdown) {
		...
		if (FLAGS_compact_delete_sst_keys > 0) {
			...
			for (const auto& [sst_file_name, prop] : props) {
				...
				if (FLAGS_compact_delete_sst_keys > 0 && prop->num_deletions > FLAGS_compact_delete_sst_keys) {
					TimeCost cost;
...
void Store::start_db_statistics() {
	...
	while (!_shutdown) {
		...
		DB_WARNING("get properties cost: %ld, concurrency:"
		           ...
		           cost.get_time(), con.snapshot_load_concurrency.count(), con.recieve_add_peer_concurrency.count(),
		           con.add_peer_concurrency.count(), con.service_write_concurrency.count(),
		           con.new_sign_read_concurrency.count());
	...
	if (!ranges.empty() && approx_sizes.size() == ranges.size()) {
		...
		for (size_t i = 0; i < region_ids.size(); ++i) {
			if (region_sizes[i] == UINT64_MAX && idx < approx_sizes.size()) {
				...
				DB_NOTICE("region_id: %ld, size:%lu region_num_line:%ld", region_ids[i], region_sizes[i],
				          region->get_num_table_lines());
			}
		}
	}
	return 0;
...
void Store::process_heart_beat_response(const pb::StoreHeartBeatResponse& response) {
	...
	for (int i = 0; i < response.trans_leader_table_id_size(); ++i) {
		table_trans_leader_count[response.trans_leader_table_id(i)] = response.trans_leader_count(i);
	}
	...
	auto remove_func = [this, remove_queue_items]() {
		...
		for (auto& remove_item : remove_queue_items) {
			...
			if (region->region_uuid() != remove_item.region_uuid()) {
				DB_WARNING("delete queue region not match delete_region_id:%ld", remove_item.drop_region_id());
				continue;
			}
...
void Store::print_heartbeat_info(const pb::StoreHeartBeatRequest& request) {
	SELF_TRACE("heart beat request(instance_info):%s, need_leader_balance: %d, need_peer_balance: %d",
	           request.instance_info().ShortDebugString().c_str(), request.need_leader_balance(),
...
void Store::backup_region(google::protobuf::RpcController* controller, const pb::BackUpReq* request,
...
void Store::backup(google::protobuf::RpcController* controller, const pb::BackupRequest* request,
...
void Store::get_rocks_statistic(google::protobuf::RpcController* controller, const pb::RocksStatisticReq* request,
