...
void DMLClosure::Run() {
			...
			if (transaction->txn_id() != 0) {
				if (status().error_code() != EPERM) {
					...
					if (op_type != pb::OP_COMMIT && op_type != pb::OP_ROLLBACK) {
						...
						DB_WARNING("txn rollback region_id: %ld log_id:%lu txn_id: %lu:%d, op_type: %s", region_id,
						           log_id, transaction->txn_id(), seq_id, pb::OpType_Name(op_type).c_str());
					}
				} else {
					DB_WARNING("leader changed region_id: %ld log_id:%lu txn_id: %lu:%d, op_type: %s", region_id,
					           log_id, transaction->txn_id(), transaction->seq_id(), pb::OpType_Name(op_type).c_str());
				}
			} else {
				// 1pc状态机外执行失败rollback
				transaction->rollback();
				DB_WARNING("txn rollback 1pc region_id: %ld log_id:%lu op_type: %s", region_id, log_id,
				           pb::OpType_Name(op_type).c_str());
			}
		}
		DB_WARNING("region_id: %ld  status:%s ,leader:%s, log_id:%lu, remote_side: %s", region_id,
		           status().error_cstr(), butil::endpoint2str(leader).c_str(), log_id, remote_side.c_str());
	} else {
	...
	if (raft_cost > FLAGS_print_time_us) {
		DB_NOTICE("dml log_id:%lu, txn_id:%lu, type:%s, raft_total_cost:%ld, region_id: %ld, "
		          ...
		          log_id, txn_id, pb::OpType_Name(op_type).c_str(), raft_cost, region_id, applied_index, is_separate,
		          (region != nullptr) ? region->num_prepared() : 0, remote_side.c_str());
	}
	if (region != nullptr) {
...
void AddPeerClosure::Run() {
	if (!status().ok()) {
		DB_WARNING("region add peer fail, new_instance:%s, status:%s, region_id: %ld, cost:%ld", new_instance.c_str(),
		           status().error_cstr(), region->get_region_id(), cost.get_time());
		if (response) {
			response->set_errcode(pb::NOT_LEADER);
		...
		DB_WARNING("region add peer success, region_id: %ld, cost:%ld", region->get_region_id(), cost.get_time());
	}
...
void SplitClosure::Run() {
	...
	if (!status().ok()) {
		DB_FATAL("split step(%s) fail, region_id: %ld status:%s, time_cost:%ld", step_message.c_str(),
		         region->get_region_id(), status().error_cstr(), cost.get_time());
		split_fail = true;
	} else if (ret < 0) {
		DB_FATAL("split step(%s) fail, region_id: %ld, cost:%ld", step_message.c_str(), region->get_region_id(),
		         cost.get_time());
		split_fail = true;
	} else {
		split_status.reset();
		...
		DB_WARNING("last step(%s) for split, start to next step, "
		           ...
		           step_message.c_str(), region->get_region_id(), cost.get_time());
	}
	auto remove_region = [](int64_t old_region_id, int64_t region_id, std::string leader,
	                        std::vector<std::string> peers) {
		DB_WARNING("split fail, start remove region, old_region_id: %ld, split_region_id: %ld, peer:%s", old_region_id,
...
void ConvertToSyncClosure::Run() {
	if (!status().ok()) {
		DB_FATAL("region_id: %ld, asyn step exec fail, status:%s, time_cost:%ld", region_id, status().error_cstr(),
		         cost.get_time());
	} else {
		DB_WARNING("region_id: %ld, asyn step exec success, time_cost: %ld", region_id, cost.get_time());
	}
