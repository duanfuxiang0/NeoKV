...
void Region::query(google::protobuf::RpcController* controller,
    ...
    if (!is_leader()) {
        ...
        if (request->extra_req().use_read_idx()) {
            pb::ErrCode err = follower_read_wait(log_id, FLAGS_follow_read_timeout_s * 1000 * 1000LL);
            if (err != pb::SUCCESS) {
                response->set_errcode(err);
                if (err == pb::NOT_LEADER) {
...
void Region::apply_redis_write(const pb::StoreReq& request, braft::Closure* done,
                               int64_t index, int64_t term) {
    TimeCost cost;
    ...
    // Get table/index IDs - use fixed Redis index_id
    int64_t index_id = get_table_id();  // In Redis mode, table_id == index_id
    if (index_id <= 0 || !is_redis_table(index_id)) {
        // Use fixed Redis index_id (no SQL table needed)
        index_id = REDIS_INDEX_ID;
    }
    ...
    for (const auto& kv : redis_req.kvs()) {
        ...
        std::string rocks_key = RedisCodec::encode_key(
            _region_id, index_id, slot, user_key, REDIS_STRING);
    ...
    DB_DEBUG("Redis write done: cmd=%d, kvs=%d, affected=%ld, region_id=%ld, cost=%ld",
             cmd, redis_req.kvs_size(), num_affected, _region_id, cost.get_time());
}
