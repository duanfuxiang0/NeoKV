...
int MetaWriter::init_meta_info(const pb::RegionInfo& region_info) {
	...
	if (!status.ok()) {
		DB_FATAL("write init_meta_info fail, err_msg: %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	}
	return 0;
}
...
int MetaWriter::update_region_info(const pb::RegionInfo& region_info) {
	...
	if (!region_info.SerializeToString(&string_region_info)) {
		DB_FATAL("region_info: %s serialize to string fail, region_id: %ld", region_info.ShortDebugString().c_str(),
		         region_id);
	}
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(region_info_key(region_id)),
	                            rocksdb::Slice(string_region_info));
	if (!status.ok()) {
		DB_FATAL("write update_region_info fail, err_msg: %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	} else {
...
int MetaWriter::update_region_ddl_info(const pb::StoreRegionDdlInfo& region_ddl_info) {
	...
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(region_ddl_info_key(region_id)),
	                            rocksdb::Slice(string_region_ddl_info));
	if (!status.ok()) {
		DB_FATAL("write update_region_ddl_info fail, err_msg: %s, region_id: %ld", status.ToString().c_str(),
		         region_id);
		return -1;
	} else {
...
int MetaWriter::update_num_table_lines(int64_t region_id, int64_t num_table_lines) {
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(num_table_lines_key(region_id)),
	                            rocksdb::Slice(encode_num_table_lines(num_table_lines)));
	if (!status.ok()) {
		DB_FATAL("write update_num_table_lines fail, err_msg: %s, region_id: %ld", status.ToString().c_str(),
		         region_id);
		return -1;
	} else {
...
int MetaWriter::update_apply_index(int64_t region_id, int64_t applied_index, int64_t data_index) {
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(applied_index_key(region_id)),
	                            rocksdb::Slice(encode_applied_index(applied_index, data_index)));
	if (!status.ok()) {
		DB_FATAL("write apply index fail, err_msg: %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	} else {
...
int MetaWriter::update_rollup_region_init_index(int64_t region_id, int64_t rollup_region_init_index) {
	auto status =
	    _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(rollup_region_init_index_key(region_id)),
...
int MetaWriter::write_pre_commit(int64_t region_id, uint64_t txn_id, int64_t num_table_lines, int64_t applied_index) {
	...
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(pre_commit_key(region_id, txn_id)),
	                            rocksdb::Slice(line_value.data()));
	if (!status.ok()) {
		DB_FATAL("write pre commit fail, err_msg: %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	}
	return 0;
}
int MetaWriter::write_doing_snapshot(int64_t region_id) {
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(doing_snapshot_key(region_id)),
	                            rocksdb::Slice(""));
	if (!status.ok()) {
		DB_FATAL("write doing snapshot fail, err_msg: %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	}
	return 0;
}
int MetaWriter::write_batch(rocksdb::WriteBatch* updates, int64_t region_id) {
	...
	if (!status.ok()) {
		DB_FATAL("write batch fail, err_msg: %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	}
	return 0;
}
...
int MetaWriter::read_transcation_rollbacked_tag(int64_t region_id, uint64_t txn_id) {
	...
	auto status =
	    _rocksdb->get(options, _meta_cf, rocksdb::Slice(rollbacked_transcation_key(region_id, txn_id)), &value);
	if (!status.ok()) {
		return -1;
	}
	return 0;
...
int MetaWriter::write_meta_after_commit(int64_t region_id, int64_t num_table_lines, int64_t applied_index,
                                        int64_t data_index, uint64_t txn_id, bool need_write_rollback) {
	if (applied_index == 0) {
		return 0;
	}
...
int MetaWriter::ingest_meta_sst(const std::string& meta_sst_file, int64_t region_id) {
	...
	if (!res.ok()) {
		DB_WARNING("SstFileReader open fail %s, Error %s, region_id: %ld", meta_sst_file.c_str(),
		           res.ToString().c_str(), region_id);
		return -1;
	}
	rocksdb::ReadOptions read_opt;
	std::unique_ptr<rocksdb::Iterator> iter(reader.NewIterator(read_opt));
	for (iter->Seek(""); iter->Valid(); iter->Next()) {
		auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, iter->key(), iter->value());
		if (!status.ok()) {
			DB_FATAL("put meta fail, err_msg: %s, region_id: %ld", status.ToString().c_str(), region_id);
			return -1;
		}
	}
	return 0;
...
int MetaWriter::clear_meta_info(int64_t drop_region_id) {
	...
	if (!status.ok()) {
		DB_FATAL("drop region fail, error: code=%d, msg=%s, region_id: %ld", status.code(), status.ToString().c_str(),
		         drop_region_id);
		return -1;
	}
...
int MetaWriter::clear_txn_log_index(int64_t region_id) {
	...
	auto status = _rocksdb->remove_range(MetaWriter::write_options, _meta_cf, start_key, end_key, false);
	if (!status.ok()) {
		DB_WARNING("remove_range error: code=%d, msg=%s, region_id: %ld", status.code(), status.ToString().c_str(),
		           region_id);
		return -1;
	}
	return 0;
...
int MetaWriter::parse_region_infos(std::vector<pb::RegionInfo>& region_infos) {
	...
	for (iter->Seek(region_info_prefix); iter->Valid(); iter->Next()) {
		...
		if (!region_info.ParseFromString(iter->value().ToString())) {
			DB_FATAL("parse from pb fail when load region snapshot, key:%s", iter->value().ToString().c_str());
			continue;
		}
...
int MetaWriter::parse_txn_infos(int64_t region_id, std::map<int64_t, std::string>& prepared_txn_infos) {
	...
	for (iter->Seek(prefix); iter->Valid(); iter->Next()) {
		...
		if (iter->key().size() == (1 + sizeof(int64_t) + 1 + sizeof(uint64_t) + sizeof(int64_t))) {
			...
			DB_WARNING("parse txn info, log_index: %ld, txn_id: %lu, region_id: %ld", log_index,
			           TableKey(iter->key()).extract_u64(1 + sizeof(int64_t) + 1), region_id);
		} else {
			log_index = TableKey(iter->key()).extract_i64(1 + sizeof(int64_t) + 1);
			DB_WARNING("parse txn info, log_index: %ld, region_id: %ld", log_index, region_id);
		}
...
void MetaWriter::read_applied_index(int64_t region_id, const rocksdb::ReadOptions& options, int64_t* applied_index,
	...
	if (!status.ok()) {
		DB_WARNING("Error while read applied index, Error %s, region_id: %ld", status.ToString().c_str(), region_id);
		*applied_index = -1;
		*data_index = -1;
		return;
	}
...
int64_t MetaWriter::read_num_table_lines(int64_t region_id) {
	...
	if (!status.ok()) {
		DB_WARNING("Error while read num_table_lines fail, Error %s, region_id: %ld", status.ToString().c_str(),
		           region_id);
		return -1;
	}
	return TableKey(rocksdb::Slice(value)).extract_i64(0);
...
int MetaWriter::read_learner_key(int64_t region_id) {
	...
	if (!status.ok()) {
		DB_WARNING("Error while read learner key, Error %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	}
...
int MetaWriter::read_region_info(int64_t region_id, pb::RegionInfo& region_info) {
	...
	if (!region_info.ParseFromString(value)) {
		DB_FATAL("parse from pb fail when read region info, region_id: %ld, value:%s", region_id, value.c_str());
		return -1;
	}
	return 0;
}
...
int MetaWriter::read_region_ddl_info(int64_t region_id, pb::StoreRegionDdlInfo& region_ddl_info) {
	...
	if (!region_ddl_info.ParseFromString(value)) {
		DB_FATAL("parse from pb fail when read region ddl info, region_id: %ld, value:%s", region_id, value.c_str());
		return -1;
	}
	return 0;
}
...
int MetaWriter::read_pre_commit_key(int64_t region_id, uint64_t txn_id, int64_t& num_table_lines,
	...
	DB_WARNING("region_id: %ld read pre commit value, txn_id: %lu num_table_lines: %ld, applied_index: %ld", region_id,
	           txn_id, num_table_lines, applied_index);
	return 0;
}
...
int MetaWriter::write_binlog_check_point(int64_t region_id, int64_t ts) {
	MutTableKey value;
	value.append_i64(ts);
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(binlog_check_point_key(region_id)),
	                            rocksdb::Slice(value.data()));
	if (!status.ok()) {
		DB_FATAL("write binlog check point fail, err_msg: %s, region_id: %ld, ts: %ld", status.ToString().c_str(),
		         region_id, ts);
		return -1;
	}
	return 0;
...
int MetaWriter::write_learner_key(int64_t region_id, bool is_learner) {
	DB_DEBUG("write learner key region %ld, is_learner %d write %s", region_id, is_learner,
	         str_to_hex(encode_learner_flag(is_learner)).c_str());
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(learner_key(region_id)),
	                            rocksdb::Slice(encode_learner_flag(is_learner)));
	if (!status.ok()) {
		DB_FATAL("write binlog check point fail, err_msg: %s, region_id: %ld, learner: %d", status.ToString().c_str(),
		         region_id, is_learner);
		return -1;
	}
	return 0;
}
...
int64_t MetaWriter::read_binlog_check_point(int64_t region_id) {
	...
	if (!status.ok()) {
		DB_FATAL("Error while read binlog check point, Error %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	}
	return TableKey(rocksdb::Slice(value)).extract_i64(0);
}
...
int MetaWriter::put_skip_watt_stats_version(int64_t region_id, uint64_t raft_index) {
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf,
	                            rocksdb::Slice(skip_watt_stats_version_key(region_id, raft_index)), rocksdb::Slice(""));
	if (!status.ok()) {
...
int MetaWriter::write_olap_info(int64_t region_id, const pb::OlapRegionInfo& olap_info) {
	...
	if (!olap_info.SerializeToString(&string_olap_info)) {
		DB_FATAL("olap_info: %s serialize to string fail, region_id: %ld", olap_info.ShortDebugString().c_str(),
		         region_id);
		string_olap_info.clear();
		return -1;
	}

	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(olap_key(region_id)),
	                            rocksdb::Slice(string_olap_info));
	if (!status.ok()) {
		DB_FATAL("write olap info fail, err_msg: %s, region_id: %ld, info: %s", status.ToString().c_str(), region_id,
...
int MetaWriter::read_olap_info(int64_t region_id, pb::OlapRegionInfo& olap_info) {
	...
	if (status.ok()) {
		if (!olap_info.ParseFromString(value)) {
			DB_FATAL("parse from pb fail when read olap info, region_id: %ld, value:%s", region_id, value.c_str());
			return -1;
		}
		return 0;
		...
		DB_FATAL("Error while read olap info, Error %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	}
}
...
int MetaWriter::write_column_file_info(int64_t region_id, const pb::RegionColumnFiles& file_info) {
	...
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(key), rocksdb::Slice(value));
	if (!status.ok()) {
		DB_FATAL("write olap info fail, err_msg: %s, info: %s", status.ToString().c_str(),
		         file_info.ShortDebugString().c_str());
		return -1;
	}
	return 0;
}
...
int MetaWriter::clear_column_file_info(int64_t region_id) {
	...
	auto status = _rocksdb->write(options, &batch);
	if (!status.ok()) {
		DB_FATAL("drop column file info fail, error msg=%s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	}
	return 0;
}
...
int MetaWriter::write_binlog_oldest_ts(int64_t region_id, int64_t ts) {
	MutTableKey value;
	value.append_i64(ts);
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(binlog_oldest_ts_key(region_id)),
	                            rocksdb::Slice(value.data()));
	if (!status.ok()) {
		DB_FATAL("write binlog oldest ts fail, err_msg: %s, region_id: %ld, ts: %ld", status.ToString().c_str(),
		         region_id, ts);
		return -1;
	}
	return 0;
...
int64_t MetaWriter::read_binlog_oldest_ts(int64_t region_id) {
	...
	if (!status.ok()) {
		DB_FATAL("Error while read binlog oldest ts, Error %s, region_id: %ld", status.ToString().c_str(), region_id);
		return -1;
	}
	return TableKey(rocksdb::Slice(value)).extract_i64(0);
}
...
int MetaWriter::write_binlog_data_cf_oldest_ts(int64_t region_id, int64_t ts) {
	...
	value.append_i64(ts);
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf,
	                            rocksdb::Slice(binlog_data_cf_oldest_ts_key(region_id)), rocksdb::Slice(value.data()));
	if (!status.ok()) {
		DB_FATAL("write binlog data cf oldest ts fail, err_msg: %s, region_id: %ld, ts: %ld", status.ToString().c_str(),
		         region_id, ts);
		return -1;
	}
	return 0;
...
int64_t MetaWriter::read_binlog_data_cf_oldest_ts(int64_t region_id) {
	...
	if (!status.ok()) {
		DB_WARNING("no binlog data cf oldest ts found, status: %s, region_id: %ld", status.ToString().c_str(),
		           region_id);
		return -1;
	}
	return TableKey(rocksdb::Slice(value)).extract_i64(0);
...
int MetaWriter::write_region_offline_binlog_info(int64_t region_id,
	...
	auto status = _rocksdb->put(MetaWriter::write_options, _meta_cf, rocksdb::Slice(offline_binlog_key(region_id)),
	                            rocksdb::Slice(string_offline_binlog_info));
	if (!status.ok()) {
		DB_FATAL("write offline_binlog_info fail, err_msg: %s, region_id: %ld, info: %s", status.ToString().c_str(),
		         region_id, offline_binlog_info.ShortDebugString().c_str());
		return -1;
	}
	return 0;
}
...
int MetaWriter::read_region_offline_binlog_info(int64_t region_id, pb::RegionOfflineBinlogInfo& offline_binlog_info) {
	...
	if (status.ok()) {
		if (!offline_binlog_info.ParseFromString(value)) {
			DB_FATAL("parse from pb fail when read offline_binlog_info, region_id: %ld, value:%s", region_id,
			         value.c_str());
			return -1;
		}
		return 0;
		...
		DB_FATAL("Error while read offline_binlog_info, Error %s, region_id: %ld", status.ToString().c_str(),
		         region_id);
		return -1;
	}
}
...
int MetaWriter::rocks_hang_check() {
	...
	auto status = _rocksdb->put(write_option, _meta_cf, rocksdb::Slice(key.data()), rocksdb::Slice(value.data()));
	if (!status.ok()) {
