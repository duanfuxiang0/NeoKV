// Enable full BaikalDB integration for Redis commands
...
// Conditional includes for full BaikalDB integration
#ifdef BAIKALDB_REDIS_FULL
#include "store.h"
#include "region.h"
...
std::string get_redis_advertise_addr() {
#ifdef BAIKALDB_REDIS_FULL
    std::string ip = baikaldb::FLAGS_redis_advertise_ip;
    int port = baikaldb::FLAGS_redis_advertise_port;
    ...
    if (port <= 0) {
        port = baikaldb::FLAGS_redis_port;
    }
...
private:
    brpc::RedisCommandHandlerResult handle_keyslot(const std::vector<butil::StringPiece>& args,
        ...
        output->SetInteger(baikaldb::redis_slot(key));
        return brpc::REDIS_CMD_HANDLED;
    }
    ...
    brpc::RedisCommandHandlerResult handle_info(const std::vector<butil::StringPiece>& /*args*/,
        ...
        auto* store = baikaldb::Store::get_instance();
        int known_nodes = 1;
        int cluster_size = 1;
        ...
        if (store != nullptr) {
            ...
            store->traverse_region_map([&](const baikaldb::SmartRegion& region) {
    ...
    brpc::RedisCommandHandlerResult handle_slots(const std::vector<butil::StringPiece>& /*args*/,
        ...
        int advertise_port = baikaldb::FLAGS_redis_port;
        ...
        if (store != nullptr) {
            store->traverse_region_map([&](const baikaldb::SmartRegion& region) {
                ...
                if (!start_key.empty() && start_key.size() >= 2) {
                    ...
                    start_slot = baikaldb::KeyEncoder::to_endian_u16(slot_be);
                }
                if (!end_key.empty() && end_key.size() >= 2) {
                    uint16_t slot_be;
                    memcpy(&slot_be, end_key.data(), sizeof(slot_be));
                    end_slot = baikaldb::KeyEncoder::to_endian_u16(slot_be);
...
protected:
    static std::string store_addr_to_redis_addr(const std::string& store_addr) {
        ...
        int redis_port = baikaldb::FLAGS_redis_advertise_port > 0
                ? baikaldb::FLAGS_redis_advertise_port
                : baikaldb::FLAGS_redis_port;
    ...
    bool check_route(const std::vector<butil::StringPiece>& args,
                     brpc::RedisReply* output,
                     baikaldb::RedisRouteResult& route) {
        auto* router = baikaldb::RedisRouter::get_instance();
        route = router->route(args);

        switch (route.status) {
            case baikaldb::RedisRouteResult::OK:
                return true;
            case baikaldb::RedisRouteResult::CROSSSLOT:
                output->SetError(route.error_msg.c_str());
                return false;
            case baikaldb::RedisRouteResult::CLUSTERDOWN:
                // Standalone mode: no regions exist, use fixed IDs for testing
                // This allows testing Redis without full cluster setup
                route.status = baikaldb::RedisRouteResult::OK;
                ...
                return true;
            case baikaldb::RedisRouteResult::NOT_LEADER:
                output->SetError(route.error_msg.c_str());
                return false;
            case baikaldb::RedisRouteResult::INVALID_CMD:
                output->SetError(route.error_msg.c_str());
                return false;
            default:
    ...
    bool check_read_consistency(const baikaldb::RedisRouteResult& route,
                                brpc::RedisReply* output) {
        ...
        if (baikaldb::FLAGS_use_read_index) {
            int64_t timeout_us = static_cast<int64_t>(baikaldb::FLAGS_follow_read_timeout_s) * 1000 * 1000LL;
            if (timeout_us <= 0) {
                timeout_us = 10 * 1000 * 1000LL;
            }
            baikaldb::pb::ErrCode err = route.region->follower_read_wait(0, timeout_us);
            if (err == baikaldb::pb::SUCCESS) {
                return true;
            }
            // ReadIndex failed - check specific error
            if (err == baikaldb::pb::LEARNER_NOT_READY) {
                output->SetError("CLUSTERDOWN Learner not ready for read");
                return false;
            }
    ...
    int64_t get_index_id() {
        ...
        if (index_id <= 0) {
            ...
            return baikaldb::REDIS_INDEX_ID;
        }
        return index_id;
    }
    ...
    int read_key(int64_t region_id, int64_t index_id, uint16_t slot,
        ...
        std::string rocks_key = baikaldb::RedisCodec::encode_key(
            region_id, index_id, slot, user_key, baikaldb::REDIS_STRING);
        ...
        int64_t expire_ms = 0;
        std::string payload;
        if (!baikaldb::RedisCodec::decode_value(rocks_value, &expire_ms, &payload)) {
            DB_WARNING("Failed to decode Redis value");
            return -1;
        }

        // Check expiration
        if (baikaldb::RedisCodec::is_expired(expire_ms, baikaldb::RedisCodec::current_time_ms())) {
    ...
    int write_through_raft(const baikaldb::RedisRouteResult& route,
                           baikaldb::pb::RedisCmd cmd,
        ...
        for (const auto& kv : kvs) {
            ...
            if (expire_ms != baikaldb::REDIS_NO_EXPIRE) {
        ...
        if (route.region != nullptr) {
            ...
            if (ret != 0) {
                ...
                if (response.errcode() == baikaldb::pb::NOT_LEADER) {
        ...
        for (const auto& kv : kvs) {
            std::string rocks_key = baikaldb::RedisCodec::encode_key(
                route.region_id, index_id, route.slot, kv.first, baikaldb::REDIS_STRING);
            
            switch (cmd) {
                case baikaldb::pb::REDIS_SET:
                case baikaldb::pb::REDIS_MSET: {
                    std::string rocks_value = baikaldb::RedisCodec::encode_value(expire_ms, kv.second);
                    batch.Put(rocks->get_data_handle(), rocks_key, rocks_value);
                    ++affected;
                    break;
                }
                case baikaldb::pb::REDIS_DEL: {
                    rocksdb::ReadOptions read_options;
                    std::string existing;
                    ...
                    if (status.ok()) {
                        int64_t exp = 0;
                        std::string payload;
                        if (baikaldb::RedisCodec::decode_value(existing, &exp, &payload) &&
                            !baikaldb::RedisCodec::is_expired(exp, baikaldb::RedisCodec::current_time_ms())) {
                ...
                case baikaldb::pb::REDIS_EXPIRE:
                case baikaldb::pb::REDIS_EXPIREAT: {
                    rocksdb::ReadOptions read_options;
                    std::string existing;
                    auto status = rocks->get(read_options, rocks->get_data_handle(), rocks_key, &existing);
                    if (status.ok()) {
                        int64_t old_exp = 0;
                        std::string payload;
                        if (baikaldb::RedisCodec::decode_value(existing, &old_exp, &payload) &&
                            !baikaldb::RedisCodec::is_expired(old_exp, baikaldb::RedisCodec::current_time_ms())) {
                            std::string new_value = baikaldb::RedisCodec::encode_value(expire_ms, payload);
...
public:
    brpc::RedisCommandHandlerResult Run(const std::vector<butil::StringPiece>& args,
        ...
        if (ret == 1) {
            ...
            int64_t ttl = baikaldb::RedisCodec::compute_ttl_seconds(expire_at_ms);
...
public:
    brpc::RedisCommandHandlerResult Run(const std::vector<butil::StringPiece>& args,
        ...
        int64_t expire_at_ms = baikaldb::REDIS_NO_EXPIRE;
        ...
        for (size_t i = 3; i < args.size(); ++i) {
            if (equals_ignore_case(args[i], "ex") && i + 1 < args.size()) {
                try {
                    ...
                    expire_at_ms = baikaldb::RedisCodec::current_time_ms() + seconds * 1000;
                } catch (...) {
                ...
                try {
                    ...
                    expire_at_ms = baikaldb::RedisCodec::current_time_ms() + ms;
                } catch (...) {
                    output->SetError("ERR value is not an integer or out of range");
                    return brpc::REDIS_CMD_HANDLED;
                }
        ...
        int ret = write_through_raft(route, baikaldb::pb::REDIS_SET, kvs, expire_at_ms, output);
        if (ret == 0) {
            output->SetStatus("OK");
        }
...
public:
    brpc::RedisCommandHandlerResult Run(const std::vector<butil::StringPiece>& args,
        ...
        int64_t deleted = 0;
        int ret = write_through_raft(route, baikaldb::pb::REDIS_DEL, kvs, 
                                     baikaldb::REDIS_NO_EXPIRE, output, &deleted);
        if (ret == 0) {
...
public:
    brpc::RedisCommandHandlerResult Run(const std::vector<butil::StringPiece>& args,
        ...
        int ret = write_through_raft(route, baikaldb::pb::REDIS_MSET, kvs,
                                     baikaldb::REDIS_NO_EXPIRE, output);
        if (ret == 0) {
            output->SetStatus("OK");
        }
        // Error already set by write_through_raft
...
public:
    brpc::RedisCommandHandlerResult Run(const std::vector<butil::StringPiece>& args,
        ...
        int64_t new_expire = baikaldb::RedisCodec::current_time_ms() + seconds * 1000;
        std::vector<std::pair<std::string, std::string>> kvs;
        kvs.emplace_back(user_key, "");
        
        int64_t affected = 0;
        ret = write_through_raft(route, baikaldb::pb::REDIS_EXPIRE, kvs, new_expire, output, &affected);
        if (ret == 0) {
...
#ifdef BAIKALDB_REDIS_FULL
    // Phase 1: Data commands (requires full BaikalDB integration)
