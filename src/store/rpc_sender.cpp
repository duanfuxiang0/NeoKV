...
int RpcSender::send_no_op_request(const std::string& instance, int64_t recevie_region_id, int64_t request_version,
                                  int times) {
	int ret = 0;
	for (int i = 0; i < times; ++i) {
		...
		if (ret < 0) {
			DB_WARNING("send no op fail, region_id: %ld, reqeust: %s", recevie_region_id,
...
int RpcSender::get_peer_applied_index(const std::string& peer, int64_t region_id, int64_t& applied_index,
                                      int64_t& dml_latency) {
...
void RpcSender::get_peer_snapshot_size(const std::string& peer, int64_t region_id, uint64_t* data_size,
                                       uint64_t* meta_size, int64_t* snapshot_index) {
	pb::GetAppliedIndex request;
	request.set_region_id(region_id);
	pb::StoreRes response;
...
int RpcSender::send_query_method(const pb::StoreReq& request, const std::string& instance, int64_t receive_region_id) {
	...
	if (ret == 0) {
		if (time_cost.get_time() > FLAGS_print_time_us) {
			DB_WARNING("send request to new region success,"
			           ...
			           pb2json(response).c_str(), receive_region_id, time_cost.get_time());
		}
		return 0;
	}
	return -1;
...
int RpcSender::send_query_method(const pb::StoreReq& request, pb::StoreRes& response, const std::string& instance,
...
int RpcSender::send_async_apply_log(const pb::BatchStoreReq& request, pb::BatchStoreRes& response,
                                    const std::string& instance, butil::IOBuf* attachment_data) {
	uint64_t log_id = butil::fast_rand();
	TimeCost time_cost;
	StoreInteract store_interact(instance);
...
int RpcSender::send_init_region_method(const std::string& instance, const pb::InitRegion& init_region_request,
                                       pb::StoreRes& response) {
	StoreInteract store_interact(instance);
