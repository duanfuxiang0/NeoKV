...
int RegionControl::remove_data(int64_t drop_region_id) {
	...
	TimeCost cost;
	auto res = rocksdb->remove_range(options, data_cf, start_key.data(), end_key.data(), true);
	if (!res.ok()) {
		DB_WARNING("remove_range error: code=%d, msg=%s, region_id: %ld", res.code(), res.ToString().c_str(),
		           drop_region_id);
		return -1;
	}
...
int RegionControl::remove_data(int64_t drop_region_id, int64_t drop_index_id) {
	...
	auto res = rocksdb->remove_range(options, data_cf, start_key.data(), end_key.data(), true);
	if (!res.ok()) {
		DB_WARNING("remove_range error: code=%d, msg=%s, region_id: %ld, index_id: %ld", res.code(),
		           res.ToString().c_str(), drop_region_id, drop_index_id);
		return -1;
	}
...
int RegionControl::remove_expired_offline_data(int64_t region_id, int64_t table_id, int64_t ttl_ts, int64_t newest_ts) {
	...
	// cold_data_cf, delete < oldest_ts
	{
		...
		auto s = rocksdb->remove_cold_range(begin, end);
		if (!s.ok()) {
			DB_FATAL("region_id: %ld, table_id: %ld, ttl_ts: %ld, %s delete cold_data_cf files in range failed %s, "
			         "cost: %ld",
	...
	// cold_data_cf, delete >= newest_ts
	{
		...
		auto s = rocksdb->remove_cold_range(begin, end);
		if (!s.ok()) {
			DB_FATAL("region_id: %ld, table_id: %ld, newest_ts: %ld, %s delete cold_data_cf files in range failed %s, "
			         "cost: %ld",
	...
	DB_WARNING("binlog region_id: %ld, table_id: %ld, ttl_ts: %ld, %s, newest_ts: %ld, %s delete expired offline data "
	           "in range, cost: %ld",
...
void RegionControl::compact_data(int64_t region_id) {
	...
	if (!res.ok()) {
		DB_WARNING("region_id:%ld, compact_range error: code=%d, msg=%s", region_id, res.code(),
		           res.ToString().c_str());
	}
...
int RegionControl::remove_log_entry(int64_t drop_region_id) {
	...
	auto status =
	    rocksdb->remove_range(options, rocksdb->get_raft_log_handle(), start_key.data(), end_key.data(), true);
	if (!status.ok()) {
	...
	if (iter->Valid()) {
		...
		DB_WARNING("remove raft log entry, region_id: %ld, cost:%ld, log_index:%ld, type:%d, %s", drop_region_id,
		           cost.get_time(), log_index, head.type, req.ShortDebugString().c_str());
	}
	return 0;
}
...
int RegionControl::ingest_data_sst(const std::string& data_sst_file, int64_t region_id, bool move_files) {
	...
	if (!res.ok()) {
		DB_WARNING("ingest file %s fail, Error %s, region_id: %ld", data_sst_file.c_str(), res.ToString().c_str(),
		           region_id);
		if (!FLAGS_allow_blocking_flush) {
			...
			if (!res.ok()) {
				DB_WARNING("Error while adding file %s, Error %s, region_id: %ld", data_sst_file.c_str(),
...
void RegionControl::raft_control(google::protobuf::RpcController* controller, const pb::RaftControlRequest* request,
	...
	if (make_region_status_doing() == 0) {
		DB_WARNING("region status to doing becase of raft control, region_id: %ld, request:%s", _region_id,
		           pb2json(*request).c_str());
...
void RegionControl::add_peer(const pb::AddPeer& add_peer, SmartRegion region, ExecutionQueue& queue) {
	...
	auto init_and_add_peer = [add_peer, new_instance, region, cost]() {
		...
		DB_WARNING("start init_region, region_id: %ld, wait_time:%ld", region->get_region_id(), cost.get_time());
...
void RegionControl::add_peer(const pb::AddPeer* request, pb::StoreRes* response, google::protobuf::Closure* done) {
	brpc::ClosureGuard done_guard(done);
	...
	DB_WARNING("region status to doing because of add peer of request, region_id: %ld", _region_id);
	if (legal_for_add_peer(*request, response) != 0) {
		reset_region_status();
		return;
	}
...
int RegionControl::transfer_leader(const pb::TransLeaderRequest& trans_leader_request, SmartRegion region,
	...
	auto transfer_call = [this, region, new_leader]() {
		...
		reset_region_status();
		if (ret != 0) {
			DB_WARNING("node:%s %s transfer leader fail", region->_node.node_id().group_id.c_str(),
...
int RegionControl::init_region_to_store(const std::string instance_address, const pb::InitRegion& init_region_request,
	...
	if (response.errcode() == pb::CONNECT_FAIL) {
		DB_FATAL("add peer fail when connect new instance, region_id: %ld, instance:%s", _region_id,
	...
	if (response.errcode() != pb::SUCCESS) {
		DB_WARNING("add peer fail when init region,"
		           ...
		           region_id, instance_address.c_str(), pb::ErrCode_Name(response.errcode()).c_str());
		return -1;
	}
	DB_WARNING("send init region to store:%s success, region_id: %ld, cost: %ld", instance_address.c_str(), _region_id,
	           cost.get_time());
	return 0;
}
...
int RegionControl::legal_for_add_peer(const pb::AddPeer& add_peer, pb::StoreRes* response) {
	...
	if (!_region->_region_info.can_add_peer() || (!add_peer.is_split() && _region->_region_info.version() < 1)) {
		DB_WARNING("region_id: %ld can not add peer, can_add_peer:%d, region_version:%ld", _region_id,
		           _region->_region_info.can_add_peer(), _region->_region_info.version());
		if (response != NULL) {
	...
	for (auto& old_peer : add_peer.old_peers()) {
		...
		if (iter == str_peers.end()) {
			DB_FATAL("old_peer not equal to list peers, region_id: %ld, old_peer:%s", _region_id,
			         pb2json(add_peer).c_str());
			if (response != NULL) {
				response->set_errcode(pb::PEER_NOT_EQUAL);
...
void RegionControl::node_add_peer(const pb::AddPeer& add_peer, const std::string& new_instance, pb::StoreRes* response,
                                  google::protobuf::Closure* done) {
	DB_WARNING("_leader_add_peer, region_id: %ld ,new_instance: %s", _region_id, new_instance.c_str());
	// send add_peer to self
	Concurrency::get_instance()->add_peer_concurrency.increase_wait();
	DB_WARNING("_leader_add_peer, region_id: %ld ,new_instance: %s wait sucess", _region_id, new_instance.c_str());
	AddPeerClosure* add_peer_done = new AddPeerClosure(Concurrency::get_instance()->add_peer_concurrency);
	...
	DB_WARNING("_leader_add_peer, region_id: %ld ,new_instance: %s send sucess", _region_id, new_instance.c_str());
}
