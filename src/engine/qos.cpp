...
namespace neokv {
DEFINE_int64(qps_statistics_minutes_ago, 60,
             "qps_statistics_minutes_ago, default: 1h"); // 默认以前一小时的统计信息作为参考
...
QosBthreadLocal::QosBthreadLocal(QosType type, const uint64_t sign, const int64_t index_id)
    : _qos_type(type), _sign(sign), _index_id(index_id) {
...
void QosReject::wheather_need_reject() {
	...
	DB_WARNING("count: %d, window_count: %d, need_reject: %d, match_reject_condition: %d, time: %ld", count,
	           window_count, _has_reject, match_reject_condition, _reject_begin_time.get_time());
...
int StoreQos::mark_need_reject_sql() {
	...
	for (auto it = sql_with_multi_index.rbegin(); it != sql_with_multi_index.rend(); it++) {
		...
		if (sql_it != ptr->end()) {
			DB_WARNING("sql use multi index need reject sign: %lu, log_term [get: %ld, scan: %ld], short_term [get: "
			           "%ld, scan: %ld]",
			           it->first, it->second.get_long_term, it->second.scan_long_term, it->second.get_short_term,
			           it->second.scan_short_term);
