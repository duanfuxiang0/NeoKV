...
rocksdb::Status Transaction::Get(const rocksdb::ReadOptions& options, rocksdb::ColumnFamilyHandle* column_family,
...
void Transaction::MultiGet(const rocksdb::ReadOptions& options, rocksdb::ColumnFamilyHandle* column_family,
...
rocksdb::Status Transaction::Merge(rocksdb::ColumnFamilyHandle* column_family, const rocksdb::Slice& key,
                                   const rocksdb::Slice& value, const bool assume_tracked) {
	COLD_DB_CHECK_WRITE;
	static thread_local int64_t total_time = 0;
	static thread_local int64_t count = 0;
	TimeCost cost;
