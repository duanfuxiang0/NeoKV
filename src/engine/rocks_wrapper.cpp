...
DEFINE_int32(addpeer_rate_limit_level, 1,
             "addpeer_rate_limit_level; "
...
RocksWrapper::RocksWrapper()
    : _is_init(false), _txn_db(nullptr), _raft_cf_remove_range_count("raft_cf_remove_range_count"),
...
int32_t RocksWrapper::init(const std::string& path) {
	...
	// todo
	_log_cf_option.prefix_extractor.reset(rocksdb::NewFixedPrefixTransform(sizeof(int64_t) + 1));
	...
	_binlog_cf_option.prefix_extractor.reset(rocksdb::NewFixedPrefixTransform(sizeof(int64_t)));
	...
	_data_cf_option.prefix_extractor.reset(rocksdb::NewFixedPrefixTransform(sizeof(int64_t) * 2));
	...
	if (FLAGS_key_point_collector_interval > 0) {
		_data_cf_option.table_properties_collector_factories.emplace_back(new KeyPointsTblPropCollectorFactory());
	}
	...
	_meta_info_option.prefix_extractor.reset(rocksdb::NewFixedPrefixTransform(1));
	...
	if (s.ok()) {
		...
		s = rocksdb::TransactionDB::Open(db_options, txn_db_options, path, column_family_desc, &handles, &_txn_db);
		if (s.ok()) {
		...
		bth.run([this]() {
			...
			if (FLAGS_real_delete_old_binlog_cf) {
				...
				if (!res.ok()) {
					...
					if (!res.ok()) {
						DB_FATAL("destroy old binlog column_family failed, err_message:%s", res.ToString().c_str());
					}
				}
			} else {
				_old_binlog_cf = nullptr;
	...
	if (0 == _column_families.count(RAFT_LOG_CF)) {
		...
		if (s.ok()) {
			...
			DB_FATAL("create column family fail, column family:%s, err_message:%s", RAFT_LOG_CF.c_str(),
			         s.ToString().c_str());
			return -1;
		}
	}
	if (0 == _column_families.count(DATA_CF)) {
		...
		if (s.ok()) {
			...
			DB_FATAL("create column family fail, column family:%s, err_message:%s", DATA_CF.c_str(),
			         s.ToString().c_str());
			return -1;
		}
	}
	if (0 == _column_families.count(METAINFO_CF)) {
		...
		if (s.ok()) {
			...
			DB_FATAL("create column family fail, column family:%s, err_message:%s", METAINFO_CF.c_str(),
			         s.ToString().c_str());
			return -1;
		}
	}
...
int32_t RocksWrapper::init_cold_rocksdb(const std::string& path) {
	...
	// prefix length: regionid(8 Bytes) tableid(8 Bytes)
	_cold_option.prefix_extractor.reset(rocksdb::NewFixedPrefixTransform(sizeof(int64_t) * 2));
	...
	_cold_option.table_properties_collector_factories.emplace_back(new KeyPointsTblPropCollectorFactory());
	...
	if (s.ok()) {
		...
		s = rocksdb::TransactionDB::Open(db_options, rocksdb::TransactionDBOptions(), path, cf_desc, &handles,
		                                 &_cold_txn_db);
		if (s.ok()) {
			DB_WARNING("reopen db:%s success", path.c_str());
			for (auto& handle : handles) {
	...
	if (_cold_column_family == nullptr) {
		...
		if (s.ok()) {
			...
			DB_FATAL("create column family fail, column family:%s, err_message:%s", COLD_DATA_CF.c_str(),
			         s.ToString().c_str());
			return -1;
		}
	}
	if (_cold_binlog_cf == nullptr) {
		...
		if (s.ok()) {
			...
			DB_FATAL("create cold binlog cf fail, column family:%s, err_message:%s", COLD_BINLOG_CF.c_str(),
			         s.ToString().c_str());
			return -1;
		}
	}
...
void RocksWrapper::set_table_options(rocksdb::BlockBasedTableOptions& table_options,
	...
	if (rocksdb_gflags.rocks_use_partitioned_index_filters()) {
		...
		table_options.block_cache = rocksdb::NewLRUCache(rocksdb_gflags.rocks_block_cache_size_mb() * 1024 * 1024LL, 8,
		                                                 false, rocksdb_gflags.rocks_high_pri_pool_ratio());
	} else {
		table_options.data_block_index_type = rocksdb::BlockBasedTableOptions::kDataBlockBinaryAndHash;
		if (rocksdb_gflags.rocks_use_hyper_clock_cache()) {
#if ROCKSDB_MAJOR >= 7
			auto cache_opt = rocksdb::HyperClockCacheOptions(rocksdb_gflags.rocks_block_cache_size_mb() * 1024 * 1024LL,
			                                                 rocksdb_gflags.rocks_block_size(), 8);
			cache_opt.metadata_charge_policy =
			    rocksdb::kDontChargeCacheMetadata; // cache会比rocks_block_cache_size_mb多占用少量内存
			table_options.block_cache = cache_opt.MakeSharedCache();
#endif
		} else {
			table_options.block_cache =
			    rocksdb::NewLRUCache(rocksdb_gflags.rocks_block_cache_size_mb() * 1024 * 1024LL, 8);
		}
	}
...
rocksdb::Status RocksWrapper::remove_range(const rocksdb::WriteOptions& options,
                                           rocksdb::ColumnFamilyHandle* column_family, const rocksdb::Slice& begin,
                                           const rocksdb::Slice& end, bool delete_files_in_range) {
...
int32_t RocksWrapper::get_binlog_value(int64_t ts, std::string& binlog_value) {
	std::string key;
	uint64_t ts_endian = KeyEncoder::to_endian_u64(KeyEncoder::encode_i64(ts));
...
void RocksWrapper::update_oldest_ts_in_binlog_cf() {
	...
	uint64_t endian_ts = KeyEncoder::to_endian_u64(KeyEncoder::encode_i64(0));
...
int32_t RocksWrapper::delete_column_family(std::string cf_name) {
	...
	if (!res.ok()) {
		DB_FATAL("drop column_family %s failed, err_message:%s", cf_name.c_str(), res.ToString().c_str());
		return -1;
	}
	res = _txn_db->DestroyColumnFamilyHandle(cf_handler);
	if (!res.ok()) {
		DB_FATAL("destroy column_family %s failed, err_message:%s", cf_name.c_str(), res.ToString().c_str());
		return -1;
	}
...
int32_t RocksWrapper::create_column_family(std::string cf_name) {
	...
	if (s.ok()) {
		...
		DB_FATAL("create column family %s fail, err_message:%s", cf_name.c_str(), s.ToString().c_str());
		return -1;
	}
	_column_families[cf_name] = cf_handler;
	return 0;
...
void RocksWrapper::adjust_option(std::map<std::string, std::string> new_options) {
	...
	for (auto& pair : new_options) {
		...
		if (_defined_options.find(flag) == _defined_options.end() || _defined_options[flag] != pair.second) {
			options_changed = true;
			// 需要是SetDBOptions
			if (flag == "rocks_max_background_compactions" || flag == "rocks_max_subcompactions" ||
			    flag == "max_background_jobs") {
				db_new_options[option_name] = pair.second;
				continue;
			}
			// 需要是SetOptions
			if (flag == "rocks_hard_pending_compaction_g" || flag == "rocks_soft_pending_compaction_g") {
