...
public:
	RaftControlDone(google::protobuf::RpcController* controller, const pb::RaftControlRequest* request,
	                pb::RaftControlResponse* response, google::protobuf::Closure* done, braft::Node* node,
	                std::shared_ptr<UpdateRegionStatus> auto_reset)
	    : _controller(controller), _request(request), _response(response), _done(done), _node(node),
	...
	virtual void Run() {
		...
		if (status().ok()) {
			DB_NOTICE("raft control success, type:%d, region_id:%lu, remote_side: %s, log_id:%lu", _request->op_type(),
			          _request->region_id(), butil::endpoint2str(cntl->remote_side()).c_str(), log_id);
			_response->set_errcode(pb::SUCCESS);
		} else {
			DB_WARNING("raft control failed, type:%d, region_id:%lu,"
			           " status:%d:%s, remote_side: %s, log_id:%lu",
			           _request->op_type(), _request->region_id(), status().error_code(), status().error_cstr(),
...
void _set_peer(google::protobuf::RpcController* controller, const pb::RaftControlRequest* request,
               pb::RaftControlResponse* response, google::protobuf::Closure* done, braft::Node* node,
               std::shared_ptr<UpdateRegionStatus> auto_reset);
int _diff_peers(const std::vector<braft::PeerId>& old_peers, const std::vector<braft::PeerId>& new_peers,
                braft::PeerId* peer);

void _trans_leader(google::protobuf::RpcController* controller, const pb::RaftControlRequest* request,
                   pb::RaftControlResponse* response, google::protobuf::Closure* done, braft::Node* node);

void common_raft_control(google::protobuf::RpcController* controller, const pb::RaftControlRequest* request,
                         pb::RaftControlResponse* response, google::protobuf::Closure* done, braft::Node* node) {
	...
	case pb::GetLeader: {
		...
		if (leader_addr != butil::EndPoint()) {
			...
			DB_FATAL("node:%s %s get leader fail, log_id:%lu", node->node_id().group_id.c_str(),
			         node->node_id().peer_id.to_string().c_str(), log_id);
	...
	default:
		DB_FATAL("node:%s %s unsupport request type:%d, log_id:%lu", node->node_id().group_id.c_str(),
		         node->node_id().peer_id.to_string().c_str(), request->op_type(), log_id);
		// UpdateRegionStatus::get_instance()->reset_region_status(request->region_id());
		return;
	}
}
void _set_peer(google::protobuf::RpcController* controller, const pb::RaftControlRequest* request,
	...
	if (is_force) {
		...
		if (!status.ok()) {
			DB_FATAL("node:%s %s set peer fail, status:%d %s, ret:%s", node->node_id().group_id.c_str(),
			         node->node_id().peer_id.to_string().c_str(), status.error_code(), status.error_cstr(),
			         status.error_cstr());
			response->set_errcode(pb::INTERNAL_ERROR);
	...
	if (!status.ok() && status.error_code() == 1) {
		...
		DB_WARNING("node:%s %s list peers fail, not leader, status:%d %s, log_id: %lu",
		           node->node_id().group_id.c_str(), node->node_id().peer_id.to_string().c_str(), status.error_code(),
		           status.error_cstr(), log_id);
		return;
	}
	if (!status.ok()) {
		response->set_errcode(pb::PEER_NOT_EQUAL);
		response->set_errmsg("node list peer fail");
		DB_WARNING("node:%s %s list peers fail, status:%d %s, log_id: %lu", node->node_id().group_id.c_str(),
		           node->node_id().peer_id.to_string().c_str(), status.error_code(), status.error_cstr(), log_id);
		return;
	}
	if (inner_peers.size() != old_peers.size()) {
		DB_WARNING("peer size is not equal when set peer, node:%s %s,"
		           ...
		           node->node_id().group_id.c_str(), node->node_id().peer_id.to_string().c_str(), inner_peers.size(),
		           old_peers.size(), butil::endpoint2str(cntl->remote_side()).c_str(), log_id);
		response->set_errcode(pb::PEER_NOT_EQUAL);
	...
	for (auto& inner_peer : inner_peers) {
		...
		if (iter == old_peers.end()) {
			DB_WARNING("old_peer not equal to list peers, node:%s %s, inner_peer: %s, log_id: %lu",
			           node->node_id().group_id.c_str(), node->node_id().peer_id.to_string().c_str(),
			           butil::endpoint2str(inner_peer.addr).c_str(), log_id);
			response->set_errcode(pb::PEER_NOT_EQUAL);
	...
	if (new_peers.size() == old_peers.size() + 1) {
		if (0 == _diff_peers(old_peers, new_peers, &peer)) {
			...
			DB_FATAL("node:%s %s set peer fail, log_id:%lu", node->node_id().group_id.c_str(),
			         node->node_id().peer_id.to_string().c_str(), log_id);
			// UpdateRegionStatus::get_instance()->reset_region_status(request->region_id());
			return;
		}
		...
		if (0 == _diff_peers(old_peers, new_peers, &peer)) {
			...
			for (auto iter : status.stable_followers) {
				if (iter.first == peer) {
					...
					if (iter.second.consecutive_error_times > braft::FLAGS_raft_election_heartbeat_factor) {
						DB_WARNING("node:%s %s peer:%s is faulty,log_id:%lu", node->node_id().group_id.c_str(),
						           node->node_id().peer_id.to_string().c_str(), iter.first.to_string().c_str(), log_id);
						other_faulty = true;
					}
				}
			}
		...
		DB_NOTICE("node:%s %s, set_peer argument failed, log_id:%lu", node->node_id().group_id.c_str(),
		          node->node_id().peer_id.to_string().c_str(), log_id);
		// UpdateRegionStatus::get_instance()->reset_region_status(request->region_id());
	}
}
void _trans_leader(google::protobuf::RpcController* controller, const pb::RaftControlRequest* request,
	...
	if (ret != 0) {
		response->set_errcode(pb::NOT_LEADER);
		response->set_leader(butil::endpoint2str(node->leader_id().addr).c_str());
		DB_WARNING("node:%s %s transfer leader fail, log_id:%lu", node->node_id().group_id.c_str(),
		           node->node_id().peer_id.to_string().c_str(), log_id);
		return;
	}
	response->set_errcode(pb::SUCCESS);
