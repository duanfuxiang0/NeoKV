...
MyRaftLogStorage::MyRaftLogStorage(int64_t region_id, RocksWrapper* db, rocksdb::ColumnFamilyHandle* raftlog_handle,
                                   rocksdb::ColumnFamilyHandle* binlog_handle)
    : _first_log_index(0), _last_log_index(0), _region_id(region_id), _db(db), _raftlog_handle(raftlog_handle),
...
braft::LogStorage* MyRaftLogStorage::new_instance(const std::string& uri) const {
	...
	if (raftlog_handle == NULL) {
		DB_FATAL("get raft log handle from rocksdb fail,uri:%s, region_id: %ld", uri.c_str(), region_id);
		return NULL;
	}
	...
	if (is_binlog) {
		...
		if (binlog_handle == NULL) {
			DB_FATAL("get bin log handle from rocksdb fail,uri:%s, region_id: %ld", uri.c_str(), region_id);
			return NULL;
		}
		DB_WARNING("region_id: %ld is binlog region", region_id);
	}

	braft::LogStorage* instance =
	    new (std::nothrow) MyRaftLogStorage(region_id, rocksdb, raftlog_handle, binlog_handle);
	if (instance == NULL) {
		DB_FATAL("new log_storage instance fail, region_id: %ld", region_id);
	}
...
int MyRaftLogStorage::init(braft::ConfigurationManager* configuration_manager) {
	...
	if (_raftlog_handle == NULL) {
		DB_FATAL("raft init state is not right, handle is null, region_id: %ld", _region_id);
		return -1;
	}
	...
	rocksdb::Status status = _db->get(rocksdb::ReadOptions(), _raftlog_handle,
	                                  rocksdb::Slice(log_meta_key, LOG_META_KEY_SIZE), &string_first_log_index);
	// region is new
	if (!status.ok() && status.IsNotFound()) {
		rocksdb::WriteOptions write_option;
		// write_option.sync = true;
		// write_option.disableWAL = true;
		rocksdb::Status put_res =
		    _db->put(write_option, _raftlog_handle, rocksdb::Slice(log_meta_key, LOG_META_KEY_SIZE),
		             rocksdb::Slice((char*)&first_log_index, sizeof(int64_t)));
		if (!put_res.ok()) {
			DB_WARNING("update first log index to rocksdb fail, region_id: %ld, err_mes:%s", _region_id,
			           put_res.ToString().c_str());
			return -1;
		}
	} else if (!status.ok()) {
		DB_FATAL("read log meta info from rocksdb wrong, region_id: %ld, err_mes:%s", _region_id,
	...
	while (iter->Valid()) {
		...
		if (expected_index != index) {
			...
			DB_FATAL("Found a hole in region_id: %ld, expected_index:%ld, real_index:%ld, type:%d, %s", _region_id,
			         expected_index, index, head.type, req.ShortDebugString().c_str());
			return -1;
		}
		// decode value
		rocksdb::Slice value = iter->value();
		...
		if (_term_map.append(braft::LogId(index, head.term)) != 0) {
			DB_FATAL("fail to append term_map, region_id: %ld, index:%ld, term:%ld", _region_id, index, head.term);
			return -1;
		}
		if (head.type == braft::ENTRY_TYPE_CONFIGURATION) {
			...
			if (_parse_meta(entry, value) != 0) {
				DB_FATAL("Fail to parse meta at index:%ld, region_id: %ld", index, _region_id);
				return -1;
			}
			...
			DB_WARNING("begin add configuration, index:%ld, term:%ld, peers:%s, region_id: %ld", index, head.term,
			           peers_str.c_str(), _region_id);
	...
	DB_WARNING("region_id: %ld, first_log_index:%ld, last_log_index:%ld, time_cost: %ld", _region_id,
	           _first_log_index.load(), _last_log_index.load(), time_cost.get_time());
	return 0;
}
...
braft::LogEntry* MyRaftLogStorage::get_entry(const int64_t index) {
	...
	rocksdb::Status status =
	    _db->get(rocksdb::ReadOptions(), _raftlog_handle, rocksdb::Slice(buf, LOG_DATA_KEY_SIZE), &value);
	if (!status.ok()) {
		DB_WARNING("get index:%ld from rocksdb fail, region_id: %ld", index, _region_id);
		return NULL;
	}
	if (value.size() < LOG_HEAD_SIZE) {
		DB_FATAL("value of log index:%ld of region id:%ld is corrupted", index, _region_id);
		return NULL;
	}
	...
	case braft::ENTRY_TYPE_NO_OP:
		...
		DB_WARNING("log storage enty is no op, region_id: %ld, log_index:%ld", _region_id, index);
		break;
	default:
		DB_FATAL("Unknown entry type, log index:%ld of region id:%ld", index, _region_id);
		entry->Release();
		entry = NULL;
		break;
	}
...
int64_t MyRaftLogStorage::get_term(const int64_t index) {
	int64_t term = 0;
	{
		...
		if (index < _first_log_index.load() || index > _last_log_index.load()) {
...
int MyRaftLogStorage::append_entries(const std::vector<braft::LogEntry*>& entries, braft::IOMetric* metric) {
	...
	if (_last_log_index.load() + 1 != entries.front()->id.index) {
		DB_FATAL("There's gap betwenn appending entries and _last_log_index,"
		         ...
		         _last_log_index.load(), entries.front()->id.index, entries.front()->id.term, _region_id);
		return -1;
	}
	static thread_local int64_t total_time = 0;
	static thread_local int64_t count = 0;
	TimeCost time_cost;
	ON_SCOPE_EXIT(([&time_cost]() {
	...
	for (auto iter = entries.begin(); iter != entries.end(); ++iter) {
		if (_build_key_value(kv_raftlog_vec, kv_binlog_vec, *iter, arena) != 0) {
			DB_FATAL("Fail to build key-value, logid:%ld:%ld, region_id: %ld", (*iter)->id.index, (*iter)->id.term,
			         _region_id);
			return -1;
		}
	}
...

	{
		...
		ON_SCOPE_EXIT([]() { Concurrency::get_instance()->raft_write_concurrency.decrease_signal(); });
		auto status = _db->write(options, &batch);
		if (!status.ok()) {
			DB_FATAL("Fail to write db, region_id: %ld, err_mes:%s", _region_id, status.ToString().c_str());
			return -1;
		}
	}
	{
...
int MyRaftLogStorage::truncate_prefix(const int64_t first_index_kept) {
	...
	DB_WARNING("Truncating region_id: %ld to first index kept:%ld from first log index:%ld", _region_id,
	...
	// write_option.sync = true;
	// write_option.disableWAL = true;
	auto status = _db->put(write_option, _raftlog_handle, rocksdb::Slice(key_buf, LOG_META_KEY_SIZE),
	...
	status = _db->remove_range(rocksdb::WriteOptions(), _raftlog_handle, rocksdb::Slice(start_key, LOG_DATA_KEY_SIZE),
	                           rocksdb::Slice(end_key, LOG_DATA_KEY_SIZE), true);
	if (!status.ok()) {
...
int MyRaftLogStorage::truncate_suffix(const int64_t last_index_kept) {
	...
	lck.unlock();
	DB_WARNING("Truncating region_id: %ld to last index kept:%ld from last log index:%ld", _region_id, last_index_kept,
	           _last_log_index.load());
	// delete from rocksdb
...
int MyRaftLogStorage::reset(const int64_t next_log_index) {
	DB_WARNING("Reseting region_id: %ld to next log index :%ld", _region_id, next_log_index);
...
int MyRaftLogStorage::_build_key_value(SlicePartsVec& kv_raftlog_vec, SlicePartsVec& kv_binlog_vec,
	...
	case braft::ENTRY_TYPE_DATA:
		...
		break;
	case braft::ENTRY_TYPE_CONFIGURATION:
		raftlog_value.parts = _construct_slice_array(head_buf, entry->peers, entry->old_peers, arena);
...
int MyRaftLogStorage::_construct_slice_array(void* head_buf, const butil::IOBuf& binlog_buf,
                                             rocksdb::SliceParts* raftlog_value, rocksdb::SliceParts* binlog_key,
                                             rocksdb::SliceParts* binlog_value, butil::Arena& arena) {
	...
	// DB_WARNING("write binlog desc: %s, region_id: %ld", binlog_pb.binlog_desc().ShortDebugString().c_str(),
	// _region_id);
	...
	uint64_t ts_tmp = KeyEncoder::to_endian_u64(KeyEncoder::encode_i64(ts));
	...
	auto binlog_slices = (rocksdb::Slice*)arena.allocate(sizeof(rocksdb::Slice) * binlog_buf.backing_block_num());
	...
	auto raftlog_slices =
	    (rocksdb::Slice*)arena.allocate(sizeof(rocksdb::Slice) * (raftlog_buf.backing_block_num() + 1));
...
rocksdb::Slice* MyRaftLogStorage::_construct_slice_array(void* head_buf, const butil::IOBuf& buf, butil::Arena& arena) {
	...
	auto slices = (rocksdb::Slice*)arena.allocate(sizeof(rocksdb::Slice) * (block_num + 1));
	if (slices == NULL) {
		DB_FATAL("Fail to allocate mem, region_id: %ld", _region_id);
		return NULL;
	}
...
rocksdb::Slice* MyRaftLogStorage::_construct_slice_array(void* head_buf, const std::vector<braft::PeerId>* peers,
                                                         ...
                                                         butil::Arena& arena) {
	auto slices = (rocksdb::Slice*)arena.allocate(sizeof(rocksdb::Slice) * (!!peers + 1));
	if (slices == NULL) {
		DB_FATAL("Fail to allocate mem, region_id: %ld", _region_id);
		return NULL;
	}
...
int MyRaftLogStorage::_encode_log_data_key(void* key_buf, size_t n, int64_t index) {
	...
	// regionId
	uint64_t region_id_tmp = KeyEncoder::to_endian_u64(KeyEncoder::encode_i64(_region_id));
	...
	memcpy((void*)((char*)key_buf + sizeof(int64_t) + 1), (void*)&index_tmp, sizeof(int64_t));
	return 0;
}
...
int MyRaftLogStorage::_decode_log_data_key(const rocksdb::Slice& data_key, int64_t& region_id, int64_t& index) {
	...
	region_id = KeyEncoder::decode_i64(KeyEncoder::to_endian_u64(region_id_tmp));
