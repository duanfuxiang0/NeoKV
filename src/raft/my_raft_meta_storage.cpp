...
MyRaftMetaStorage::MyRaftMetaStorage(int64_t region_id, RocksWrapper* db, rocksdb::ColumnFamilyHandle* handle)
    : _region_id(region_id), _db(db), _handle(handle) {
}
...
RaftMetaStorage* MyRaftMetaStorage::new_instance(const std::string& uri) const {
	...
	if (instance == NULL) {
		DB_FATAL("new raft_meta_storage instance fail, region_id: %ld", region_id);
	}
...
butil::Status MyRaftMetaStorage::set_term_and_votedfor(const int64_t term, const braft::PeerId& peer_id,
...
int MyRaftMetaStorage::save() {
	...
	if (!status.ok()) {
		PLOG(ERROR) << "Fail to save meta to region_id: " << _region_id << ", rocksdb error " << status.ToString();
		return -1;
	}

	timer.stop();
	LOG(INFO) << "save stable meta region_id: " << _region_id << " term " << _term << " votedfor "
	          << _votedfor.to_string() << " time: " << timer.u_elapsed();
	return 0;
}
