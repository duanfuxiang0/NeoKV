...
RocksdbReaderAdaptor::RocksdbReaderAdaptor(int64_t region_id, const std::string& path, RocksdbFileSystemAdaptor* rs,
                                           IteratorContextPtr context, bool is_meta_reader)
    : _region_id(region_id), _path(path), _rs(rs), _context(context), _is_meta_reader(is_meta_reader) {
...
ssize_t RocksdbReaderAdaptor::read(butil::IOPortal* portal, off_t offset, size_t size) {
	if (_closed) {
		DB_FATAL("rocksdb reader has been closed, region_id: %ld, offset: %ld", _region_id, offset);
		return -1;
	}
	if (offset < 0) {
	...
	if (!_is_meta_reader && !_context->need_copy_data) {
		_context->done = true;
		DB_WARNING("region_id: %ld need not copy data, time_cost: %ld", _region_id, time_cost.get_time());
		return 0;
	}
	...
	while (count < size) {
		...
		if (_context->is_meta_sst && _context->iter->key().compare(region_info_key) == 0) {
			...
			region_info.ParseFromArray(_context->iter->value().data_, _context->iter->value().size_);
			DB_WARNING("region_id: %ld meta_sst region_info:%s", _region_id, region_info.ShortDebugString().c_str());
		}
		...
		if (_context->is_meta_sst && _context->iter->key().starts_with(log_index_prefix)) {
			...
			auto ret = LogEntryReader::get_instance()->read_log_entry(_region_id, log_index, _context->applied_index,
			                                                          txn_id, txn_infos);
			if (ret < 0) {
				_context->done = true;
...
bool RocksdbReaderAdaptor::close() {
	if (_closed) {
		DB_WARNING("file has been closed, region_id: %ld, num_lines: %ld, path: %s", _region_id, _num_lines,
		           _path.c_str());
		return true;
	}
	_rs->close(_path);
...
SstWriterAdaptor::SstWriterAdaptor(int64_t region_id, const std::string& path, const rocksdb::Options& option)
    : _region_id(region_id), _path(path), _writer(new SstFileWriter(option)) {
}

int SstWriterAdaptor::open() {
	_region_ptr = Store::get_instance()->get_region(_region_id);
	if (_region_ptr == nullptr) {
		DB_FATAL("open sst file path: %s failed, region_id: %ld not exist", _path.c_str(), _region_id);
		return -1;
	}
...
ssize_t SstWriterAdaptor::write(const butil::IOBuf& data, off_t offset) {
	...
	if (region_shutdown()) {
		DB_FATAL("write sst file path: %s failed, region shutdown, data len: %lu, region_id: %ld", path.c_str(),
		         data.size(), _region_id);
		return -1;
	}
	if (_closed) {
		DB_FATAL("write sst file path: %s failed, file closed: %d data len: %lu, region_id: %ld", path.c_str(), _closed,
		         data.size(), _region_id);
		return -1;
	}
	if (data.size() == 0) {
	...
	if (ret < 0) {
		DB_FATAL("write sst file path: %s failed, received invalid data, data len: %lu, region_id: %ld", path.c_str(),
		         data.size(), _region_id);
		return -1;
	}
	// 大region addpeer中重置time_cost，防止version=0超时删除
	...
	DB_WARNING("rocksdb sst write, region_id: %ld, path: %s, offset: %lu, data.size: %ld,"
	           " file_size: %lu, total_count: %lu",
	           _region_id, path.c_str(), offset, data.size(), _writer->file_size(), _count);
	//}
	return data.size();
}
...
bool SstWriterAdaptor::finish_sst() {
	...
	if (_count > 0) {
		...
		if (!s.ok()) {
			DB_FATAL("finish sst file path: %s failed, err: %s, region_id: %ld", path.c_str(), s.ToString().c_str(),
			         _region_id);
			return false;
		}
	} else {
...
int SstWriterAdaptor::iobuf_to_sst(butil::IOBuf data) {
	...
	while (!data.empty()) {
		...
		if (key.data_ == nullptr) {
			DB_FATAL("read key from iobuf fail, region_id: %ld, key_size: %ld", _region_id, key_size);
			return -1;
		}
		...
		if (value.data_ == nullptr) {
			DB_FATAL("read value from iobuf fail, region_id: %ld, value_size: %ld", _region_id, value_size);
			return -1;
		}
		...
		if (_is_meta) {
			if (key == region_info_key) {
				...
				DB_WARNING("region_id: %ld meta_sst recv region_info:%s", _region_id,
		...
		if (!s.ok()) {
			DB_FATAL("write sst file failed, err: %s, region_id: %ld", s.ToString().c_str(), _region_id);
			return -1;
		}
	}
	return 0;
...
bool RocksdbFileSystemAdaptor::create_directory(const std::string& path, butil::File::Error* error,
...
bool RocksdbFileSystemAdaptor::open_snapshot(const std::string& path) {
	...
	if (iter != _snapshots.end()) {
		...
		if (iter->second.cost.get_time() > 3600 * 1000 * 1000LL && iter->second.count == 1 &&
		    (iter->second.ptr->data_context == nullptr || iter->second.ptr->data_context->offset == 0)) {
	...
	DB_WARNING("region_id: %ld, data_index:%ld, binlog_check_point:%ld, open snapshot path: %s", _region_id,
	           _snapshots[path].ptr->data_index, _snapshots[path].ptr->binlog_check_point, path.c_str());
	return true;
}
...
void RocksdbFileSystemAdaptor::close(const std::string& path) {
	...
	if (iter == _snapshots.end()) {
		DB_FATAL("no snapshot found when close reader, path:%s, region_id: %ld", path.c_str(), _region_id);
		return;
	}
