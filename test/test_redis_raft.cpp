...
TEST_F(RedisCodecExtendedTest, EmptyKey) {
    ...
    EXPECT_TRUE(RedisCodec::decode_key(encoded, &dec_region, &dec_index, 
                                       &dec_slot, &dec_key, &dec_type));
    EXPECT_EQ("", dec_key);
}
...
TEST_F(RedisCodecExtendedTest, ExpireTimeBoundaries) {
    ...
    std::vector<int64_t> expire_times = {
        REDIS_NO_EXPIRE,
        0,
        1,
        INT64_MAX - 1,
...
TEST_F(RedisSlotExtendedTest, SlotRange) {
    ...
    std::vector<std::string> keys = {
        "a", "abc", "test_key", "user:1000", "{tag}key",
        "very_long_key_with_many_characters_to_test_the_slot_calculation"
    };
...
TEST_F(RedisSlotExtendedTest, HashTagExtraction) {
    ...
    std::vector<std::string> tagged_keys = {
        "{user1000}.profile",
        "{user1000}.settings",
        "{user1000}:data",
        "{user1000}"
...
TEST_F(RedisSlotExtendedTest, CrossslotDetection) {
    ...
    std::vector<std::string> different_keys = {
        "key_a", "key_b", "key_c", "user:1", "user:2"
    };
...
TEST_F(RedisStressTest, ManyKeysEncodeDecode) {
    ...
    for (int i = 0; i < num_keys; ++i) {
        ...
        ASSERT_TRUE(RedisCodec::decode_key(rocks_key, &dec_region, &dec_index,
                                           &dec_slot, &dec_key, &dec_type));
...
TEST_F(RedisStressTest, ProtobufSerialization) {
    ...
    std::cout << "Serialized/deserialized " << num_iterations << " requests in " 
